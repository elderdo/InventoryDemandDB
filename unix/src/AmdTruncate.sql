/*
      $Author:   c402417  $
    $Revision:   1.2  $
        $Date:   Nov 02 2007 13:55:32  $
    $Workfile:   AmdTruncate.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Components-ClientServer\Unix\AmdTruncate.sql.-arc  $
/*   
...
exec amd_owner.mta_truncate_table('tmp_a2a_bom_detail','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_demands','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_org_flight_acty','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_org_flight_acty_frecst','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_site_resp_asset_mgr','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_spo_users','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_part_effectivity','reuse storage');
...
exec amd_owner.mta_truncate_table('tmp_a2a_ext_forecast','reuse storage');
exec amd_owner.mta_truncate_table('tmp_amd_dmnd_frcst_consumables','reuse storage');
exec amd_owner.mta_truncate_table('tmp_a2a_part_alt','reuse storage');
