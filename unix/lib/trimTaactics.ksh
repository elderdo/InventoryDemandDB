...
\t-f the forecast can have these values: none, or complete (default is complete)\n
...
LOG_NAME="$LOG_HOME/${TimeStamp}_${AMD_CUR_STEP:+${AMD_CUR_STEP}_}trimTactics.log"
...
do
	case $arguments in
	  f) case ${OPTARG} in
	     	none|complete) SPO_FORECAST=${OPTARG};;
	     	*) abort "invalid forecast ${OPTARG} - valid values are none, complete" ;;
	     esac ;;
...
getTrimTacticsLogFile() {
	...
	if (($?==0)) ; then
		if [[ -n $TACTICS_LOG_FILE_NAME ]] ; then
			...
			issueDiagnostic "Warning: Unable to get the name of the tactics log file name" 
			return 4
		fi
	fi
