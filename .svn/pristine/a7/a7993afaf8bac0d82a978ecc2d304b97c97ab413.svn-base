...
*/
   ...
   begin
	...
	and  not exists (select null from tmp_locpart_overid_consumables
			...
			and tsl_override_type = 'ROP Fixed' )
	and not exists (select null from tmp_locpart_overid_consumables
			where a.part = part_no and spo_location <> 'FD2090') ;	
	...
	and  not exists (select null from tmp_locpart_overid_consumables
			where a.part = part_no and spo_location = 'FD2090'
			and tsl_override_type = 'ROQ Fixed' ) 
	and not exists (select null from tmp_locpart_overid_consumables
			where a.part = part_no and spo_location <> 'FD2090') ;	
    ...
    begin
     ...
     begin
        ...
        for rec in consumables loop
            result := true ;
            /* all a2a routines have been removed from package amd_location_part_override_pkg
            result := amd_location_part_override_pkg.insertedTmpA2ALPO (
                  pPartNo    => rec.part_no,
    ...
    begin
       ...
       if partBaseRecs.first is not null then
           for indx in partBaseRecs.first .. partBaseRecs.last loop
                in_cnt := in_cnt + 1 ;
                tsl_override_user := getTslOverrideUser(partBaseRecs(indx).spo_prime_part_no ) ;
    ...
    begin
        writeMsg(pTableName => 'amd_lp_override_consumabl_pkg',
             pError_location => 730, pKey1 => 'amd_lp_override_consumabl_pkg', pKey2 => '$Revision:   1.84  $') ;
        dbms_output.put_line('amd_lp_override_consumabl_pkg: $Revision:   1.84  $') ;
    end version ;
