    ...
    exception when standard.no_data_found then
     ...
     procedure insertTmpA2APartInfo(mfgr in tmp_a2a_part_info.cage_code%type,
        ...
        begin
            ...
            resp_asset_mgr = getAssignedPlannerCode(insertTmpA2APartInfo.part_no),
     ...
     begin
       ...
       VALUES (mfgr, part_no, unit_issue, nomenclature, rcm_ind, SUBSTR(nsn, 1, 4), SUBSTR(nsn, 5, 9),
         getAssignedPlannerCode(part_no) , THIRD_PARTY_FLAG, mtbdr, preferred_smrcode, indenture, action_code, 
         SYSDATE, price) ;
     ...
     END insertTmpA2ASpoUsers ;
      ...
      BEGIN
        IF not isPlannerCodeAssigned2UserId(plannerCode) THEN
           plannerCode := null ;             
        END IF ;
        debugMsg('For part ' || part_no || ' planner_code ' || plannerCode || ' is not assigned to a users.  Using default of ' || plannerCode, 200) ; 
     ...
     PROCEDURE initA2ABomDetail(from_dt IN DATE := START_DT, to_dt IN DATE := SYSDATE ) IS
        ...
        BEGIN
             writeMsg(pTableName => 'a2a_pkg', 
                     pError_location => 1640, pKey1 => 'a2a_pkg', pKey2 => '$Revision:   1.220  $') ;
              dbms_output.put_line('a2a_pkg: $Revision:   1.220  $') ;
        END version ;

        function getVersion return varchar2 is
        begin
            return '$Revision:   1.220  $' ;
        end getVersion ;
        ...
        procedure deleteSentToA2AChildren is
                ...
                result number ; 
        begin
            writeMsg(pTableName => 'amd_sent_to_a2a', pError_location => 1650,
            ...
            writeMsg(pTableName => 'amd_sent_to_a2a', pError_location => 1651,
                pKey1 => 'deleteSentToA2AChildren',
                pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
                pKey3 => 'cnt=' || to_char(cnt) ) ;
             commit ;
