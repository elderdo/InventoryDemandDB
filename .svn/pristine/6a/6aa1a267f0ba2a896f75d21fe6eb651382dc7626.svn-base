...
exec amd_owner.mta_truncate_table('wip_vendor_code','reuse storage');
...
INSERT into amd_owner.w_item (item_id, part, sc, location, qty, receipt_um, condition, serial,
...
SELECT
item_id, part, sc, location, qty, receipt_um, condition, serial,
status_servicable,status_suspended, status_avail, reserve_action, status_in_transit, 
status_new_order, status_mai, status_accountable, status_active, status_1
...
from (select order_no, max(created_sys_datetime) cr_sys_dt from amd_owner.w_wip1 group by order_no order by order_no) dts,
amd_owner.w_wip1
...
( item_id, master, prime, part, sc, site_code, location, qty, um, condition, serial, description, status_servicable,
...
VENDOR_CODE, STATUS, ORDER_TYPE, SC, ORIGINAL_LOCATION, LOCATION, 
...
insert into amd_owner.wip_vendor_code 
(part, noun, prime, source_code, nsn, ims_designator_code, smrc_code, vendor_code)
SELECT distinct CAT1.*, 
COALESCE(OH.VENDOR_CODE,OH2.VENDOR_CODE,ORD1.VENDOR_CODE,VC.VENDOR_CODE) VENDOR
FROM
(select part, noun, prime, source_code SOS, nsn, IMS_DESIGNATOR_CODE IMS, SMRC 
 FROM cat1@amd_pgoldlb_link CAT1
 WHERE (SOURCE_CODE = 'F77' OR (SOURCE_CODE IS NULL AND NSN = 'NSL' 
 AND PART IN (SELECT PART FROM whse@amd_pgoldlb_link WHSE WHERE 
 SC LIKE :program_id || 'GISP%')))
AND LNNVL(USER_REF7 = 'Y') AND SUBSTR(SMRC,6,1) = 'T') CAT1,
(SELECT DISTINCT ORDH1.PART, ORDH1.VENDOR_CODE FROM ordh1@amd_pgoldlb_link ORDH1 WHERE STATUS = 'A' AND wip_type like '%REPAIR%' AND VENDOR_CODE NOT IN ('0003808','0005170','5421275','0004805','0003004','0005246','0002663','5422100','3A6427','3A0068','3A6605',
'3A0681','3A0068','0002725','0001154','1A4110','5422102','5422056','5422005','5422127','FB2039','FY2362','FX2339')) OH,
(SELECT DISTINCT ORDH1.PART, ORDH1.VENDOR_CODE FROM ordh1@amd_pgoldlb_link ORDH1 WHERE STATUS = 'A' AND wip_type like '%REPAIR%' AND VENDOR_CODE IN ('0003808','0005170','5421275','0004805','0003004','0005246','0002663','5422100','3A6427','3A0068','3A6605',
'3A0681','3A0068','0002725','0001154','1A4110','5422102','5422056','5422005','5422127','FB2039','FY2362','FX2339')) OH2,
(SELECT ORDER_NO, PART, VENDOR_CODE FROM ord1@amd_pgoldlb_link ORD1 WHERE ORDER_TYPE = 'J' AND ACCOUNTABLE_YN = 'Y' AND SC LIKE :program_id || '%' 
AND SC <> :program_id || 'GISPCFTMODG' AND NH_ORDER_NO IS NULL AND
CREATED_DATETIME IN (SELECT max(CREATED_DATETIME) from ord1@amd_pgoldlb_link ord1 where ORDER_TYPE = 'J' AND ACCOUNTABLE_YN = 'Y' AND NH_ORDER_NO IS NULL AND SC LIKE :program_id || '%' AND SC <> :program_id || 'GISPCFTMODG' group by part)) ORD1,
(SELECT * 
 FROM venc@amd_pgoldlb_link VENC 
 WHERE (PART,SEQ) IN (SELECT T1.PART, MAX(T1.SEQ) FROM venc@amd_pgoldlb_link T1 GROUP BY T1.PART)) VC
WHERE
CAT1.PART = OH.PART(+) AND 
CAT1.PART = OH2.PART(+) AND
CAT1.PART = ORD1.PART(+) AND 
CAT1.PART = VC.PART(+)
order by cat1.prime, cat1.part;
