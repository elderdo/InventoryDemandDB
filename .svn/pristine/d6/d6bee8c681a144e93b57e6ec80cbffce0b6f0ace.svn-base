/* Formatted on 1/25/2017 4:38:28 PM (QP5 v5.287) */
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.Amd_Reqs_Pkg
IS
   /*
       PVCS Keywords
       ...
       $Workfile:   AMD_REQS_PKG.pkb  $
          ...
          Rev 1.24   25 Jan 2017 DSE reformatted code
  ...
  /*   fixed deleteRow and added PVCS keywords
   ...
   IS
   BEGIN
      Amd_Utils.writeMsg (pSourceName       => 'amd_reqs_pkg',
   ...
   IS
      ...
      CURSOR req1_cur
      IS
           SELECT (REQ1.request_id) request_id,
             ...
             FROM REQ1, AMD_SPARE_PARTS asp, AMD_SPARE_NETWORKS asn
            WHERE     REQ1.status IN ('U',
                                      'O',
                                      'H',
                                      'R')
                  AND REQ1.request_priority IN (1,
                                                2,
                                                3,
                                                4,
                                                5)
                  AND REQ1.mils_source_dic IS NOT NULL
                  AND asn.loc_id = SUBSTR (REQ1.request_id, 1, 6)
                  AND asp.part_no = REQ1.select_from_part
                  AND ( (NVL (REQ1.qty_due, 0) + NVL (REQ1.qty_reserved, 0)) >
                          0)
                  AND asn.action_code IN
      ...
      CURSOR tmp1_cur
      IS
           SELECT (TMP1.temp_out_id) temp_out_id,
                  (TMP1.from_part) part_no,
                  asn.loc_sid,
                  SUM (TMP1.qty_due) qty_due,
                  amd_defaults.getINSERT_ACTION action_code,
                  SYSDATE last_update_dt
             FROM TMP1, AMD_SPARE_NETWORKS asn, AMD_SPARE_PARTS asp
            WHERE     TMP1.returned_voucher IS NULL
                  AND TMP1.status = 'O'
                  AND TMP1.TCN = 'LBR'
                  AND TMP1.qty_due != 0
                  AND asn.loc_id = to_loc_id
                  AND asp.part_no = TMP1.from_part
   ...
   BEGIN
      ...
      IF reqRecs.FIRST IS NOT NULL
      THEN
         FORALL indx IN reqRecs.FIRST .. reqRecs.LAST
            INSERT INTO TMP_AMD_REQS
                 VALUES reqRecs (indx);

         DBMS_OUTPUT.put_line ('loadAmdReqs: rows inserted ' || reqRecs.LAST);
         COMMIT;
      END IF;
   ...
   FUNCTION getSiteLocation (loc_sid IN AMD_SPARE_NETWORKS.loc_sid%TYPE)
      RETURN AMD_SPARE_NETWORKS.spo_location%TYPE
   IS
   ...
   FUNCTION InsertRow (req_id         IN VARCHAR2,
                       part_no        IN VARCHAR2,
                       loc_sid        IN NUMBER,
                       quantity_due   IN NUMBER)
      RETURN NUMBER
   IS
      result   NUMBER;
   BEGIN
         ...
         BEGIN
           <<getActionCode>>
            ...
            UPDATE AMD_REQS
             ...
             WHERE req_id = InsertRow.req_id AND part_no = InsertRow.part_no;
         END doUpdate;
      BEGIN
        <<insertAmdReqs>>
         BEGIN
            INSERT INTO AMD_REQS (req_id,
                                  part_no,
                                  loc_sid,
                                  quantity_due,
                                  action_code,
   ...
   FUNCTION UpdateRow (REQ_ID         IN VARCHAR2,
                       PART_NO        IN VARCHAR2,
                       LOC_SID        IN NUMBER,
                       QUANTITY_DUE   IN NUMBER)
      RETURN NUMBER
   ...
   BEGIN
     <<updateAmdReqs>>
      BEGIN
         UPDATE AMD_REQS
            SET quantity_due = UpdateRow.quantity_due,
                action_code = Amd_Defaults.UPDATE_ACTION,
                last_update_dt = SYSDATE
          WHERE     req_id = UpdateRow.req_id
   ...
   FUNCTION DeleteRow (REQ_ID    IN VARCHAR2,
                       PART_NO   IN VARCHAR2,
                       LOC_SID   IN NUMBER)
      RETURN NUMBER
   IS
   ...
   FUNCTION InsertRowBackorder (spo_prime_part_no   IN VARCHAR2,
      ...
      BEGIN
         UPDATE AMD_BACKORDER_SUM
            SET qty = InsertRowBackorder.qty,
                action_code = Amd_Defaults.INSERT_ACTION,
                last_update_dt = SYSDATE
          WHERE     spo_prime_part_no = InsertRowBackorder.spo_prime_part_no
                AND spo_location = InsertRowBackorder.spo_location;
   ...
   BEGIN
     ...
     <<validateData>>
      ...
      BEGIN
         line_no := line_no + 1;
         rec.spo_prime_part_no := spo_prime_part_no;
         line_no := line_no + 1;
         rec.spo_location := insertRowBackorder.spo_location;
         line_no := line_no + 1;
         rec.qty := qty;
      EXCEPTION
         WHEN OTHERS
      ...
      IF (qty > 0) AND spo_prime_part_no IS NOT NULL
      THEN
         BEGIN
            INSERT INTO AMD_BACKORDER_SUM (spo_prime_part_no,
                                           spo_location,
                                           qty,
                                           action_code,
                                           last_update_dt)
                 VALUES (InsertRowBackorder.spo_prime_part_no,
   ...
   FUNCTION UpdateRowBackorder (spo_prime_part_no   IN VARCHAR2,
                                spo_location        IN VARCHAR2,
                                qty                 IN NUMBER)
      RETURN NUMBER
   IS
   ...
   BEGIN
     <<updateAmdBackorderSum>>
      BEGIN
         UPDATE AMD_BACKORDER_SUM
            SET qty = UpdateRowBackorder.qty,
                action_code = Amd_Defaults.UPDATE_ACTION,
                last_update_dt = SYSDATE
   ...
   FUNCTION DeleteRowBackorder (spo_prime_part_no   IN VARCHAR2,
                                spo_location        IN VARCHAR2)
      RETURN NUMBER
   IS
   BEGIN
     <<updateAmdBackorderSum>>
      ...
      EXCEPTION
         ...
         THEN
            writeMsg (
               pTableName        => 'updateAmdBackorderSum',
               pError_location   => 100,
               pKey1             => 'spo_prime_part_no=' || spo_prime_part_no,
               pKey2             => 'loc_sid=' || spo_location);
            RETURN success;
   ...
   FUNCTION InsertRowSpoSum (
      ...
      qty                 IN AMD_BACKORDER_SPO_SUM.QTY%TYPE)
      RETURN NUMBER
   IS
      result   NUMBER;
     ...
     <<validateData>>
      ...
      THEN
        <<insertAmdBackorderSpoSum>>
         BEGIN
            INSERT INTO AMD_BACKORDER_SPO_SUM (spo_prime_part_no,
                                               qty,
                                               action_code,
                                               last_update_dt)
                 VALUES (spo_prime_part_no,
                         qty,
                         Amd_Defaults.INSERT_ACTION,
                         SYSDATE);
         EXCEPTION
