 ...
 -- same name
    ...
    function isPartValid(partNo IN VARCHAR2, preferredSmrCode IN VARCHAR2, 
     ...
     BEGIN
           ...
           if onHandInvSums.first is not null then
               ...
               LOOP
                  A2a_Pkg.insertInvInfo(onHandInvSums(indx).part_no,rec.spo_location,onHandInvSums(indx).qty_on_hand, onHandInvSums(indx).action_code) ;
                   cnt := cnt + 1 ;
               END LOOP ;
              COMMIT ;
            end if ;              
    ...
    BEGIN
        ...
        if repairInvInfos.first is not null then         
             ...
             LOOP
                 A2a_Pkg.insertRepairInvInfo(repairInvInfos(indx).part_no,rec.site_location,repairInvInfos(indx).qty_on_hand, repairInvInfos(indx).action_code) ;
                  cnt := cnt + 1 ;
             END LOOP ;
             COMMIT ;
        end if ;             
    ...
    BEGIN
     ...
     BEGIN
           ...
           if inRepairs.first is not null then
               ...
               LOOP
                   A2a_Pkg.insertRepairInfo(inRepairs(indx).part_no,rec.loc_sid,inRepairs(indx).order_no,inRepairs(indx).repair_date,A2a_Pkg.OPEN_STATUS,inRepairs(indx).repair_qty,
    ...
    BEGIN
        ...
        EXCEPTION WHEN standard.NO_DATA_FOUND THEN
         ...
         BEGIN
            SELECT apl.time_to_repair_defaulted
                INTO time_to_repair_defaulted
                FROM amd_part_locs apl
                where nsi_sid = insertTimeToRepair.nsi_sid
                and loc_sid = 23;
            EXCEPTION WHEN standard.NO_DATA_FOUND THEN
         ...
         IF (time_to_repair IS not NULL or (time_to_repair is null and action_code = amd_defaults.DELETE_ACTION))
    ...
    begin
             OPEN overrides FOR
                  SELECT alpo.part_no,
                        ...
                        amd_location_part_override_pkg.OVERRIDE_TYPE AS override_type,
                        case
     ...
     PROCEDURE initA2ABomDetail(from_dt IN DATE := START_DT, to_dt IN DATE := SYSDATE ) IS
        ...
        BEGIN
             writeMsg(pTableName => 'a2a_pkg', 
                     pError_location => 1640, pKey1 => 'a2a_pkg', pKey2 => '$Revision:   1.208  $') ;
              dbms_output.put_line('a2a_pkg: $Revision:   1.208  $') ;
        END version ;
