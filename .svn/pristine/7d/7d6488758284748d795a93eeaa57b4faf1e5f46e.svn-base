...
END;
	...
	END doInsertTmpA2ABackOrderInfo ;
	  ...
	  BEGIN
		...
		IF (qty > 0) and spo_prime_part_no is not null THEN  
			...
			DECLARE
				site_location TMP_A2A_BACKORDER_INFO.site_location%TYPE := spo_location;
			BEGIN
				 ...
				 AND site_location IS NOT NULL THEN
					 ...
					 begin
						 if spo_prime_part_no is not null then
						 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => 999, site_location => site_location,
							 							 qty => qty, 
														 action_code => Amd_Defaults.INSERT_ACTION) ;
						 end if ;
		  ...
		  IF (qty > 0)  THEN	  
			  ...
			  BEGIN
				...
				AND A2a_Pkg.wasPartSent(spo_prime_part_no) THEN
					...
					begin
					 if spo_prime_part_no  IS NOT NULL then
					 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => 888, site_location => site_location,
						 							 qty => qty,
													 action_code => Amd_Defaults.UPDATE_ACTION) ;
					 end if ;
					end insertTmpA2ABackorderInfo ;
		...
		BEGIN
			...
			BEGIN
			    IF site_location IS NOT NULL  AND  spo_prime_part_no IS NOT NULL THEN 
				 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => 777, site_location => site_location,
					 							 qty => 0,
												 action_code => Amd_Defaults.DELETE_ACTION) ;
				END IF;
			END insertTmpA2ABackorderInfo ;
	...
	BEGIN
		 writeMsg(pTableName => 'amd_reqs_pkg', 
		 		pError_location => 110, pKey1 => 'amd_reqs_pkg', pKey2 => '$Revision:   1.19  $') ;
	END version ;
