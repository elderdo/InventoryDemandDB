...
/*   Initial revision.
    */
	...
	TYPE part2Delete IS RECORD (
	 ...
	 FUNCTION getAssignedPlannerCode(part_no IN TMP_A2A_PART_INFO.part_no%TYPE) RETURN AMD_PLANNERS.planner_code%TYPE  ;
...
/*   Initial revision.
 ...
 -- same name
    ...
    exception when standard.no_data_found then
     ...
     BEGIN
       ...
       SET
         ...
         resp_asset_mgr = getAssignedPlannerCode(part_no),
     ...
     procedure insertTmpA2APartInfo(mfgr in tmp_a2a_part_info.cage_code%type,
        ...
        begin
            ...
            nsn_fsc = SUBSTR(nsn, 1, 4), nsn_niin = SUBSTR(nsn, 5, 9), 
     ...
     begin
       debugMsg('insertTmpA2APartInfo: ' || part_no, pError_location => 990) ;
       if planner_code is null then
            thePlannerCode := amd_defaults.REPAIRABLE_PLANNER_CODE ;
       else
       ...
       VALUES (mfgr, part_no, unit_issue, nomenclature, rcm_ind, SUBSTR(nsn, 1, 4), SUBSTR(nsn, 5, 9),
         thePlannerCode , THIRD_PARTY_FLAG, mtbdr, preferred_smrcode, indenture, action_code, 
         SYSDATE, price) ;
     ...
     begin
       ...
       validateData (mfgr,part_no,unit_issue,nomenclature,smr_code,nsn,plannerCode, third_party_flag,
           mtbdr, indenture, rcm_ind) ;
           
        if isPartValid(partNo => part_no, preferredSmrCode => smr_code, 
            preferredMtbdr => mtbdr, preferredPlannerCode => plannerCode) THEN
     ...
     BEGIN
       if amd_utils.isPartConsumable(part_no) then
            a2a_consumables_pkg.insertPartInfo(amd_defaults.DELETE_ACTION, part_no, nomenclature,
	...
	is_spo_part amd_spare_parts.is_spo_part%type ;
     ...
     begin
       debugMsg('doTmpA2APartLeadTimeInsert: ' || part_no || ' ' || lead_time_type || ' ' || lead_time,
	pError_location => 1090) ;
        select is_consumable, is_repairable, is_spo_part 
	into is_consumable, is_repairable, is_spo_part
	from amd_spare_parts 
	where part_no = doTmpA2APartLeadTimeInsert.part_no ;
       IF ( ( action_code = amd_defaults.DELETE_ACTION 
		and ( ( is_repairable = 'Y' or is_consumable = 'Y' ) and isPartSent(part_no) )
            )
               or
                 (  is_spo_part = 'Y'  AND wasPartSent(part_no) )
             )  
        THEN
     ...
     PROCEDURE updateA2ApartPricing(
          ...
          result NUMBER ;
     BEGIN
       debugMsg('updateA2ApartPricing(' || part_no || ' ' || price_type || ' ' || price || ')' ,
        pError_location => 1142) ;
	...
	pError_location => 1195) ;
     ...
     PROCEDURE initSiteRespAssetMgr IS
               ...
               cntNoUser number := 0 ;
     BEGIN
       writeMsg(pTableName => 'tmp_a2a_site_resp_asset_mgr', pError_location => 1250,
       ...
       */
       writeMsg(pTableName => 'tmp_a2a_site_resp_asset_mgr', pError_location => 1260,
        pKey1 => 'initSiteRespAssetMgr',
     ...
     PROCEDURE insertSiteRespAssetMgr(
           ...
           BEGIN
            ...
            EXCEPTION WHEN OTHERS THEN
               ErrorMsg( pSqlfunction => 'update', pTableName => 'tmp_a2a_site_resp_asset_mgr', pError_location => 1270,
     ...
     BEGIN
       ...
       EXCEPTION
           WHEN standard.DUP_VAL_ON_INDEX THEN
                   doUpdate ;
         WHEN OTHERS THEN
           ErrorMsg( pSqlfunction => 'insert', pTableName => 'tmp_a2a_site_resp_asset_mgr', pError_location => 1280,
             pkey_1 => assetMgr, pKey_2 => logonId, pKey_3 => data_source) ;
           RAISE ;
          END insertTmpA2ASiteRespAssetMgr ;
    
     EXCEPTION WHEN OTHERS THEN
        ErrorMsg( pSqlfunction => 'insert', pTableName => 'tmp_a2a_site_resp_asset_mgr', pError_location => 1290,
          pkey_1 => assetMgr, pKey_2 => logonId, pKey_3 => data_source ) ;
        raise ;
     ...
     PROCEDURE insertInvInfo(part_no IN TMP_A2A_INV_INFO.part_no%TYPE,
        ...
        action_code IN TMP_A2A_INV_INFO.action_code%TYPE)  IS
         ...
         PROCEDURE doUpdate is
          ...
          EXCEPTION WHEN OTHERS THEN
            ErrorMsg( pSqlfunction => 'select', pTableName => 'tmp_a2a_inv_info', pError_location => 1300,
        ...
        EXCEPTION
           WHEN standard.DUP_VAL_ON_INDEX THEN
                doUpdate ;
           WHEN OTHERS THEN
             ErrorMsg( pSqlfunction => 'insert', pTableName => 'tmp_a2a_inv_info', pError_location => 1310,
               pkey_1 => part_no, pKey_2 => spo_location) ;
             RAISE ;
        end insertRow ;
    ...
    PROCEDURE insertRepairInvInfo(part_no IN TMP_A2A_REPAIR_INV_INFO.part_no%TYPE,
        ...
        EXCEPTION
          WHEN standard.DUP_VAL_ON_INDEX THEN
                doUpdate ;
          WHEN OTHERS THEN
             ErrorMsg( pSqlfunction => 'insert', pTableName => 'tmp_a2a_repair_inv_info', pError_location => 1330,
     ...
     -- expectedCompletionDate TMP_A2A_REPAIR_INFO.expected_completion_date%TYPE
      ...
      EXCEPTION WHEN OTHERS THEN
         errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_repair_info', pError_location => 1360,
      ...
      EXCEPTION
           WHEN standard.DUP_VAL_ON_INDEX THEN
               doUpdate ;
           WHEN OTHERS THEN
             errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_repair_info', pError_location => 1370,
         pkey_1 => part_no, pkey_2 => loc_sid, pkey_3 => doc_no, pkey_4 => TO_CHAR(repair_date,'MM/DD/YYYY'),pKeywordValuePairs => 'status=' || status || '  qty=' || quantity || ' action=' || action_code) ;
             RAISE ;
      end insertRow ;
     ...
     PROCEDURE insertTmpA2AOrderInfoLIne(gold_order_number IN AMD_ON_ORDER.GOLD_ORDER_NUMBER%TYPE,
        ...
        PROCEDURE doInsert(action_code in tmp_a2a_order_info_line.ACTION_CODE%type) IS
             ...
             EXCEPTION WHEN OTHERS THEN
                 errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_order_info_line', pError_location => 1420,
        ...
        BEGIN
             begin
                  lineOfCode := 6 ;
                  writeMsg(pTableName => 'tmp_a2a_order_info_line',pError_location => 1430,
                    pKey1 => 'gold_order_number=' || gold_order_number,
        ...
        EXCEPTION
              ...
              WHEN OTHERS THEN
                 ...
                 if sqlcode <> -4092 then
                     errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_order_info_line', pError_location => 1440,
           ...
           or insertTmpA2AOrderInfoLine.action_code = amd_defaults.UPDATE_ACTION then
               ...
               IF wasPartSent(insertTmpA2AOrderInfoLine.part_no) 
                    ...
                    ELSE
                        ...
                        writeMsg(pTableName => 'tmp_a2a_order_info_line',pError_location => 1450,
                               pKey1 => 'gold_order_number=' || gold_order_number,
     ...
     EXCEPTION
           WHEN OTHERS THEN
              if sqlcode <> -4092 then
                  errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_order_info_line', pError_location => 1460,
     ...
     -- site_location TMP_A2A_IN_TRANSITS.site_location%TYPE := Amd_Utils.getSpoLocation(to_loc_sid) ;
        ...
        EXCEPTION WHEN OTHERS THEN
          errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_in_transits', pError_location => 1470,
       ...
       -- added exception handlers DSE 11/30/05
        EXCEPTION
               WHEN standard.DUP_VAL_ON_INDEX THEN
                     doUpdate ;
              WHEN OTHERS THEN 
                  errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_in_transits', pError_location => 1480,
                    pkey_1 => insertTmpA2AInTransits.part_no, pKey_2 => insertTmpA2AInTransits.site_location) ;
                  RAISE ;
        end insertRow ;
     ...
     BEGIN
      ...
      EXCEPTION
       WHEN standard.DUP_VAL_ON_INDEX THEN
         NULL ; -- ignore
       WHEN OTHERS THEN
        errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_spo_users', pError_location => 1490,
            pkey_1 => bems_id) ;
        RAISE ;
      END insertA2A ;
     ...
     PROCEDURE insertTmpA2ASpoUsers(bems_id IN TMP_A2A_SPO_USERS.bems_id%TYPE,
           ...
           PROCEDURE doUpdate IS
                     ...
                     debugIt BOOLEAN := A2a_Pkg.debug ; -- save current debug settings
           BEGIN
                   SELECT email, name INTO theEmail, theName FROM TMP_A2A_SPO_USERS WHERE bems_id = insertTmpA2ASpoUsers.bems_id ;
                A2a_Pkg.debug := TRUE ; -- always record the before and after
                debugMsg(msg => 'before update bems_id = ' || bems_id || 'email= ' || theEmail || 'last_name=' || theName, pError_location => 1500) ;
                ...
                debugMsg(msg => 'after update bems_id = ' || bems_id || 'email= ' || SUBSTR(stable_email,1,32) || 'last_name=' || SUBSTR(last_name || ', ' || first_name,1,32), pError_location => 1510) ;
                A2a_Pkg.debug := debugIt ; -- restore
                
           EXCEPTION WHEN OTHERS THEN
                errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_spo_users', pError_location => 1520,
                    pkey_1 => bems_id) ;
                RAISE ;
           END doUpdate ;
     ...
     EXCEPTION
               WHEN standard.DUP_VAL_ON_INDEX THEN
                     doUpdate ;
              WHEN OTHERS THEN
                    errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_spo_users', pError_location => 1530,
                        pkey_1 => bems_id) ;
                    RAISE ;
     ...
     END insertTmpA2ASpoUsers ;
      ...
      FUNCTION getAssignedPlannerCode(part_no IN TMP_A2A_PART_INFO.part_no%TYPE) RETURN AMD_PLANNERS.planner_code%TYPE  IS
    ...
    PROCEDURE populateBomDetail(part_no IN TMP_A2A_BOM_DETAIL.part_no%TYPE,
              ...
              EXCEPTION WHEN OTHERS THEN
                errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_bom_detail', pError_location => 1560,
        ...
        EXCEPTION
             WHEN standard.DUP_VAL_ON_INDEX THEN
                   doUpdate ;
             WHEN OTHERS THEN
                errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_bom_detail', pError_location => 1570,
                    pkey_1 => populateBomDetail.part_no, pkey_2 => populateBomDetail.included_part) ;
                RAISE ;         
     ...
     PROCEDURE initA2ABomDetail(from_dt IN DATE := START_DT, to_dt IN DATE := SYSDATE ) IS
        ...
        PROCEDURE processBackorder(rec IN AMD_BACKORDER_SUM%ROWTYPE, site_location IN TMP_A2A_BACKORDER_INFO.site_location%TYPE) IS
           ...
           BEGIN
               ...
               AND site_location = rec.spo_location ;
           EXCEPTION WHEN OTHERS THEN
                errormsg( psqlfunction => 'update', ptablename => 'tmp_a2a_backorder_info', pError_location => 1660,
                    pkey_1 => rec.spo_prime_part_no, pkey_2 => TO_CHAR(rec.spo_location), pkey_3 => site_location) ;
                   RAISE ;
           END doUpdate ;
        ...
        BEGIN
            INSERT INTO TMP_A2A_BACKORDER_INFO
                (part_no,  site_location,qty, action_code, last_update_dt)
                VALUES
                (rec.spo_prime_part_no,  site_location, rec.qty, rec.action_code, rec.last_update_dt);
        EXCEPTION 
            WHEN standard.DUP_VAL_ON_INDEX THEN
                doUpdate ;
            WHEN OTHERS THEN
                errormsg( psqlfunction => 'insert', ptablename => 'tmp_a2a_backorder_info', pError_location => 1670,
                    pkey_1 => rec.spo_prime_part_no, pkey_2 => TO_CHAR(rec.spo_location), pkey_3 => site_location) ;
                RAISE ;
        ...
        BEGIN
           writeMsg(pTableName => 'loadAll', pError_location => 1760,
            ...
            FOR step IN startStep..endStep LOOP
                ...
                ELSIF step = 10 THEN
                     IF NOT Amd_Batch_Pkg.isStepComplete(batch_job_number => theJob, system_id => loadAll.system_id,
                         ...
                         Amd_Batch_Pkg.start_step(batch_job_number => theJob, system_id => loadAll.system_id,
                            ...
                            null ; --amd_owner.Amd_Location_Part_Leadtime_Pkg.LoadAllA2A ;
                    END IF ;
                ELSIF step = 11 THEN
                     IF NOT Amd_Batch_Pkg.isStepComplete(batch_job_number => theJob, system_id => loadAll.system_id,
                ...
                COMMIT ;
                 ...
                 END IF ;
                 COMMIT ;
                   writeMsg(pTableName => 'loadAll', pError_location => 1770,
                         pKey1 => 'step=' || step,
                        pKey2 => 'rc=' || rc) ;
                 COMMIT ;
                 rc := 0 ;
            ...
            writeMsg(pTableName => 'loadAll', pError_location => 1780,
        ...
        BEGIN
             writeMsg(pTableName => 'a2a_pkg', 
                     pError_location => 1790, pKey1 => 'a2a_pkg', pKey2 => '$Revision:   1.232  $') ;
              dbms_output.put_line('a2a_pkg: $Revision:   1.232  $') ;
        END version ;

        function getVersion return varchar2 is
        begin
            return '$Revision:   1.232  $' ;
        end getVersion ;
