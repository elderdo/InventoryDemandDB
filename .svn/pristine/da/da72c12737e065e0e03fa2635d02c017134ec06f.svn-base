...
SHOW SQLTERMINATOR
SHOW SQLBLANKLINES
PROMPT ready repairInvSumDiff.sql
SET ECHO ON
SET TIME ON
SET TIMING ON
...
DECLARE
   ...
   IS
        SELECT part_no,
               ...
               SUM (repair_qty) qty_on_hand
          FROM tmp_amd_in_repair
         WHERE     action_code != 'D'
               AND (order_no LIKE 'R%' OR order_no LIKE 'II%')
      GROUP BY part_no, Amd_Utils.getSpoLocation (loc_sid)
        HAVING Amd_Utils.getSpoLocation (loc_sid) IS NOT NULL
      ORDER BY part_no, Amd_Utils.getSpolocation (loc_sid);

   CURSOR deleteRecs
   IS
      SELECT part_no, site_location, qty_on_hand
        FROM amd_repair_invs_sum cur
       WHERE     NOT EXISTS
                        (SELECT NULL
                           FROM (  SELECT part_no,
                                 ...
                                 GROUP BY part_no,
                                          Amd_Utils.getSpoLocation (loc_sid)
                                   HAVING Amd_Utils.getSpoLocation (loc_sid)
                                             IS NOT NULL) repairInvSum
                          WHERE     repairInvSum.part_no = cur.part_no
                                AND repairInvSum.site_location =
                                       cur.site_location)
             AND action_code <> 'D';
   ...
   IS
      CURSOR curRecs (
         partNo          VARCHAR2,
         siteLocation    amd_repair_invs_sum.site_location%TYPE)
      IS
           SELECT *
      ...
      cnt      NUMBER := 0;
   BEGIN
      FOR oldRec IN curRecs (newRec.part_no, newRec.site_location)
      LOOP
   ...
   FUNCTION isInsert (partNo          amd_repair_invs_sum.part_no%TYPE,
                      siteLocation    amd_repair_invs_sum.site_location%TYPE)
      RETURN BOOLEAN
   IS
      result   BOOLEAN := FALSE;
   ...
   IS
   BEGIN
      RETURN amd_inventory.doRepairInvsSumDiff (rec.part_no,
                                                rec.site_location,
                                                rec.qty_on_hand,
                                                'A');
   END insertRow;
...
BEGIN
   ...
   LOOP
      newCnt := newCnt + 1;
      IF isInsert (newRec.part_no, newRec.site_location)
      THEN
