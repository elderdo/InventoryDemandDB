...
begin
	...
	begin
		 amd_utils.writeMsg(pSourceName => 'amd_utils', pTableName => 'amd_utils',
		 		pError_location => 999, pKey1 => 'amd_utils', pKey2 => '$Revision:   1.64  $') ;
		 dbms_output.put_line('a2a_utils: $Revision:   1.64  $') ;
	end version ;
   ...
   function isPartConsumable(part_no amd_spare_parts.part_no%type) return boolean is
	...
	function rank(orderOfUsage in varchar2) return number is
			 ...
			 exp number := 0 ;
			 cnt number := 0 ;
			 countingChars varchar2(36) := '0123456789ABCDEFGHIJILMNOPQRSTUVWXYZ' ;
