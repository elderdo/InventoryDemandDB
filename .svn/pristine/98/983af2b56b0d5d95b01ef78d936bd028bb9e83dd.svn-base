CREATE OR REPLACE PACKAGE BODY AMD_OWNER.Amd_Spare_Networks_Pkg
AS
   /*
         $Author:   zf297a  $
       $Revision:   1.9
   ...
   /*      Rev 1.9   Jan 24 2017 Douglas Elder reformated code with Toad and added 
                                 dbms_ouput to print insert counts 
   /*      Rev 1.8   Sep 21 2015 added getProgramId
   ...
   */
   ...
   IS
   BEGIN
      Amd_Utils.writeMsg (pSourceName       => 'amd_spare_networks_pkg',
   ...
   BEGIN
     ...
     <<ramp_gold_table>>
      BEGIN
         INSERT INTO AMD_SPARE_NETWORKS (loc_id,
                                         location_name,
                                         loc_type,
                                         action_code,
                                         spo_location,
                                         last_update_dt)
              SELECT DISTINCT (sran),
                              sran,
                              'UAB',
                              lv_action,
                              'VIRTUAL UAB',
                              SYSDATE
                FROM RAMP a
               WHERE     SUBSTR (sc, 1, PROGRAM_ID_LL) = PROGRAM_ID
         ...
         THEN
            DBMS_OUTPUT.put_line (
                  'auto_load_spare_networks: rows inserted from ramp '
               || SQL%ROWCOUNT);
         END IF;
      ...
      BEGIN
         INSERT INTO AMD_SPARE_NETWORKS (loc_id,
            ...
            SELECT DISTINCT
              ...
              FROM ITEM a
             WHERE     SUBSTR (sc, 1, PROGRAM_ID_LL) = PROGRAM_ID
                   AND (   SUBSTR (loc_id, 1, 3) = 'CTL'
                        OR SUBSTR (loc_id, 1, 3) = 'COD')
                   AND loc_id != 'CODLGB'
                   AND SUBSTR (sc, 14, 1) = 'G'
                   AND NOT EXISTS
                          (SELECT 'x'
         ...
         THEN
            DBMS_OUTPUT.put_line (
                  'auto_load_spare_networks: rows inserted from item '
               || SQL%ROWCOUNT);
         END IF;
      EXCEPTION
         WHEN NO_DATA_FOUND
