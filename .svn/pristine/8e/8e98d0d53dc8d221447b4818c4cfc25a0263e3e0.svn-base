 /*
      $Author:   zf297a  $
    $Revision:   1.0  $
        $Date:   23 Aug 2007 10:41:22  $
    $Workfile:   amd_partprime_pkg.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\amd_partprime_pkg.sql.-arc  $
/*   
/*      Rev 1.0   23 Aug 2007 10:41:22   zf297a
/*   Initial revision.
*/
...
/*  need to clean up and streamline logic on this package */
            ...
            exception 
                when standard.no_data_found then
        		    writeMsg(pTableName => 'updatePlannerCodesForSubParts', 
        		 		pError_location => 155, pKey1 => 'part_no=' || rec.spo_prime_part_no, pKey2 => 'planner code not found') ;
                when others then
        			  ErrorMsg(
    				   pTableName  	  	  => 'updatePlannerCodesForSubParts',
    				   pError_location 	  => 157,
    				   pKey1			  => 'part_no=' || rec.spo_prime_part_no) ;
    				   RAISE ;	  		  			  		                
            end ;        
        end loop ;        
			...
			BEGIN	
				...
				ELSE 
					 IF ( a2aRec.spo_prime_part_no != latestPrime ) THEN
						...
						END IF ;
                        amd_location_part_override_pkg.sendZeroTslsForSpoPrimePart(a2aRec.spo_prime_part_no) ;
				     END IF ;
				END IF ;  
			EXCEPTION WHEN OTHERS THEN
		 		   ErrorMsg(
	...
	BEGIN
		 writeMsg(pTableName => 'amd_partprime_pkg', 
		 		pError_location => 170, pKey1 => 'amd_partprime_pkg', pKey2 => '$Revision:   1.0  $') ;
	END version ;
