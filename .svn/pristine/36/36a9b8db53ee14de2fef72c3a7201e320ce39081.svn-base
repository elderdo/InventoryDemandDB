...
CREATE OR REPLACE PACKAGE AMD_OWNER.amd_load as
    /*
       ...
       $Author:   zf297a  $
     $Revision:   1.1  $
         $Date:   12 Sep 2007 16:12:50  $
     $Workfile:   amd_load.sql  $
	      $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\amd_load.sql.-arc  $
/*   
/*      Rev 1.1   12 Sep 2007 16:12:50   zf297a
/*   Removed commits from for loops for all scripts.
   ...
   added infoMsg, which is almost the same as ErrorMsg, but it does not do a Rollback or a commit.
		...
		FOR rec IN catCur LOOP
			...
			BEGIN
				...
				(	rec.part, rec.manuf_cage,
					...
					nsnType,			plannerCode,
					rec.um_cap_code,
					amd_utils.bizDays2CalendarDays(getOrderLeadTime(rec.part)),
	...
	begin
		 writeMsg(pTableName => 'amd_load', 
		 		pError_location => 360, pKey1 => 'amd_load', pKey2 => '$Revision:   1.1  $') ;
		 dbms_output.put_line('amd_load: $Revision:   1.1  $') ;
	end version ;
