...
/*
      $Author:   zf297a  $
    $Revision:   1.1  $
        $Date:   09 Oct 2007 15:28:16  $
    $Workfile:   a2a_consumables_pkg.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\a2a_consumables_pkg.sql.-arc  $
/*   
/*      Rev 1.1   09 Oct 2007 15:28:16   zf297a
...
*/    
    ...
    begin
        select items.smr_code, items.nsn, items.planner_code, items.mtbdr
        into smr_code, nsn, planner_code, mtbdr
    ...
    exception
        ...
        when others then
           errorMsg(pSqlfunction => 'select',pTableName => 'amd_spare_parts/amd_national_stock_items',
             pError_location => 10, pKey_1 => part_no ) ;
           raise ;
    ...
    exception when others then
       errorMsg(pSqlfunction => 'select',pTableName => 'amd_spare_parts/amd_national_stock_items',
         pError_location => 100, pKey_1 => part_no ) ;
       raise ;
	 ...
	 exception when others then
		if sqlcode = -20000 then
            ...
            errorMsg(pSqlfunction => 'select',pTableName => 'isPartValid',
              pError_location => 300, pKey_1 => part_no, pKey_2 => lineNo ) ;
            raise ;
		end if ;
	...
	procedure insertPartInfo(action_code in varchar2, part_no in varchar2, nomenclature in varchar2,
        ...
        begin
            ...
            resp_asset_mgr = insertPartInfo.planner_code,
        ...
        begin
            insert into tmp_a2a_part_info
             (cage_code, part_no, unit_issue, noun, rcm_ind, nsn_fsc, nsn_niin, resp_asset_mgr, third_party_flag,
                mtbf, preferred_smrcode, indenture, action_code, last_update_dt, price )
            values ( mfgr, part_no, unit_issue, nomenclature, rcm_ind, nsn_fsc, nsn_niin, planner_code, a2a_pkg.THIRD_PARTY_FLAG,
                mtbdr, smr_code, indenture, action_code, sysdate, price ) ;
    ...
    exception when others then
        errorMsg(pSqlfunction => 'select',pTableName => 'tmp_a2a_part_info',
            pError_location => 400, pKey_1 => part_no ) ;
        raise ;
    end insertPartInfo ;
    ...
    begin
         writeMsg(pTableName => 'a2a_consumables_pkg', 
                 pError_location => 410, pKey1 => 'a2a_consumables_pkg', pKey2 => '$Revision:   1.1  $') ;
          dbms_output.put_line('a2a_consumables_pkg: $Revision:   1.1  $') ;
    end version ;
