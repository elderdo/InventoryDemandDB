...
#   $Author:   zf297a  $
# $Revision:   1.12  $
#     $Date:   07 May 2008 17:56:02  $
# $Workfile:   profile.ksh  $
...
if [ $ENV = "DEV" ]
  then
    if [ $FROM_TRIGGER = "no" ]
      then
        ...
        do
          echo "\nPlease input your environment (dev or crp): \c"
          read AMD_SPO_ENV
 ...
 #       . /data/imacs_dev/ai_scripts/profile.sh -i DEV05 -e crp
    ...
    then
      ...
      export ORACLE_SID=tescm
    else
      MQCHANNEL=QMC1ESCMC172;export MQCHANNEL
      MQMGR=QMC1HA02;export MQMGR
      echo "Setting stl_escmdev"
      ORACLE_SID=escmdev;export ORACLE_SID
