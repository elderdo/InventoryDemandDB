...
--  Then read them with the new_part_no being passed
  ...
  END;
	...
	FUNCTION doDmndFrcstConsumablesDiff(
        ...
        begin
            if calendar_date is not null and period >= cur_period then
                ...
                and a2a_pkg.wasPartSent(part_no) then
                    ...
                    demand_forecast, action_code, sysdate, nvl(duplicate,1)) ;
    ...
    procedure loadAllA2A is
        ...
        procedure insertA2A(rec in amd_dmnd_frcst_consumables%rowtype) is
            ...
            cur_period number := getFiscalPeriod(sysdate) ;
        begin
            if rec.period >= cur_period and calendar_date is not null then
                if part_no is not null 
                and amd_utils.isPartActive(part_no) 
                and a2a_consumables_pkg.isPartValid(part_no) 
                and a2a_pkg.wasPartSent(part_no) then
                    ...
                    rec.demand_forecast, rec.action_code, sysdate, nvl(rec.duplicate,1)) ;
	...
	begin
		 writeMsg(pTableName => 'amd_demand', 
		 		pError_location => 10, pKey1 => 'amd_demand', pKey2 => '$Revision:   1.40  $') ;
	end version ;
