   ...
   added infoMsg, which is almost the same as ErrorMsg, but it does not do a Rollback or a commit.
        ...
        if catRecs.first is not null then
            FOR indx in catRecs.first .. catRecs.last LOOP
                ...
                IF (nsn != prevNsn) THEN
                    ...
                    nsnType     := 'C';
                    plannerCode := catRecs(indx).ims_designator_code;
                    itemType    := NULL;
	...
	begin
		 writeMsg(pTableName => 'amd_load', 
		 		pError_location => 360, pKey1 => 'amd_load', pKey2 => '$Revision:   1.64  $') ;
		 dbms_output.put_line('amd_load: $Revision:   1.64  $') ;
	end version ;
