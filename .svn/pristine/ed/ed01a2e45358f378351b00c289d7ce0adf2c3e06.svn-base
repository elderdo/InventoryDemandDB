...
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.amd_spare_parts_pkg as
   ...
   Added $Log$ keyword
	...
	function  getFedcCost( pPartNo varchar2) return number;

	function hasPartMoved( pPartNo varchar2, pNsn varchar2) return boolean;
	procedure unassignPart( pPartNo varchar2);

	function ErrorMsg( pSourceName in amd_load_status.SOURCE%type,
	...
	procedure debugMsg(pMsg in varchar2,pError_Location in number) is
        	pragma autonomous_transaction ;
	begin
		-- is debugging turned on for this package?
		if mDebug then
		   amd_utils.debugMsg(pMsg => pMsg,pPackage => 'amd_spare_parts', pLocation => pError_Location) ;
		   commit ; -- make sure the trace is kept
		end if ;
	end;

   procedure updateFlags(pPart_no in amd_spare_parts.part_no%type, is_spo_part out amd_spare_parts.is_spo_part%type) is
    ...
    begin
        ...
        isConsumable := 'N' ;
        isRepairable := 'N' ;
        if amd_utils.isPartRepairable(pPart_no) then
            isRepairable := 'Y' ;
            if a2a_pkg.isPartValid(pPart_no) then
                isSpoPart := 'Y' ;
            end if ;
        end if ;         
        if amd_utils.isPartConsumable(pPart_no) then
            isConsumable := 'Y' ;
            if a2a_consumables_pkg.isPartValid(pPart_no) then
                isSpoPart := 'Y' ;
            end if ;
        end if ;         
    ...
    exception when others then    
       ErrorMsg(pSqlfunction => 'updateFlags',
            pError_location => 655) ;
   end updateFlags ;
	...
	procedure unassociateTmpNsn(
							pNsn varchar2) is
	begin
		debugMsg('unassociateTmpNsn('||pNsn||')', pError_location => 10);
	...
	begin
		debugMsg('hasPartMoved('||pPartNo||')', pError_location => 20);
	...
	function  getFedcCost(
		...
		fedcCost    number;
	begin
		debugMsg('getFedcCost(' || pPartNo || ')', pError_location => 30) ;
	...
	procedure unassignPart(
							pPartNo varchar2) is
	begin
		debugMsg('unassignPart('||pPartNo||')', pError_location => 40);
	...
	begin
		debugMsg('isPrimePart(' || pPrime_ind || ')', pError_location => 50);
	...
	procedure insertNsiParts( pNsi_sid in amd_nsi_parts.nsi_sid%type,
	...
	begin
		debugMsg('insertNsiParts('||pNsi_sid||','||pPart_no||','||pPrime_ind||','||pPrime_ind_cleaned||','||pBadRc||')', pError_location => 60);
	 ...
	 * redundant code.\
	 */
	procedure MakeNsnSameForAllParts(pNsi_sid in amd_nsi_parts.nsi_sid%type,
			pNsn in amd_national_stock_items.nsn%type) is
	...
	procedure UpdateNatStkItem( pNsn in amd_spare_parts.nsn%type,
				pAction in varchar2,
				pPartNo varchar2 default null) is
	...
	begin -- UpdateNatStkItem
	    debugMsg('UpdateNatStkItem(' || pNsn || ', ' || pAction || ', ' || pPartNo || ')', pError_location => 70) ;
	 ...
	 * redundant code.
	 */
	procedure UpdtNsiPrimePartData( pPrime_ind in amd_nsi_parts.prime_ind%type,
			...
			pReturn_code in number,
			pMmac in amd_national_stock_items.mmac%type) is
		fedcCost   number;
		procedure verifyData is
				  rec amd_national_stock_items%rowtype ;
				  x number := 0 ;
		begin
		    debugMsg('verifyData', pError_location => 80) ;
		...
		begin
			debugMsg('doUpdate', pError_location => 90) ;
	...
	begin
		debugMsg('UpdtNsiPrimePartData',pError_location => 100) ;
				...
				if sqlcode = -2291 then
				   ...
				   begin
					   ...
					   if instr(sqlerrm,'FK04') > 0  then
						  if amd_utils.isPartConsumable(pPartNo) then
						  	 mPlannerCodeCleaned := amd_defaults.CONSUMABLE_PLANNER_CODE ;
						  elsif amd_utils.isPartRepairable(pPartNo) then
						     mPlannerCodeCleaned := amd_defaults.REPAIRABLE_PLANNER_CODE ;
                          else
						  ...
						  if amd_utils.isPartConsumable(pPartNo) then
						  	 planner_code := amd_defaults.CONSUMABLE_PLANNER_CODE ;
						  elsif amd_utils.isPartRepairable(pPartNo) then
						     planner_code := amd_defaults.REPAIRABLE_PLANNER_CODE ;
                          else
                             planner_code := null ;
						  end if ;
	...
	procedure InsertNatStkItem( pNsi_sid out amd_national_stock_items.nsi_sid%type,
	...
	begin -- InsertNatStkItem
	    debugMsg('InsertNatStkItem', pError_location => 110) ;
	...
	procedure ChgCurNsn2TempNsn( pNsiSid in amd_nsns.nsi_sid%type) is
	begin
	    debugMsg('ChgCurNsn2TempNsn(' || pNsiSid || ')', pError_location => 120) ;
		update amd_nsns set
	...
	procedure InsertAmdNsn(pNsi_sid in amd_nsns.nsi_sid%type,
			...
			pNsn_type in amd_nsns.nsn_type%type ) is


	begin
		debugMsg('InsertAmdNsn(' || pNsi_sid || ', ' || pNsn || ', ' || pNsn_type, pError_location => 130) ;
	...
	procedure UpdateAmdNsn(pNsi_sid in amd_nsns.nsi_sid%type,
			...
			pNsn_type in amd_nsns.nsn_type%type ) is
	begin
		debugMsg('UpdateAmdNsn(' || pNsi_sid || ', ' || pNsn || ', ' || pNsn_type || ')', pError_location => 140) ;
		if pNsn_type = amd_spare_parts_pkg.CURRENT_NSN then
		   ChgCurNsn2TempNsn(pNsiSid => pNsi_sid);
		end if ;
	...
	procedure CreateNationalStockItem( pNsi_sid out amd_national_stock_items.nsi_sid%type,
			pNsn in amd_spare_parts.nsn%type,
			pItem_type in amd_national_stock_items.item_type%type,
			pOrder_quantity in amd_national_stock_items.order_quantity%type,
			pPlanner_code in amd_national_stock_items.planner_code%type,
	...
	function InsertRow(
		...
		*/
		...
		procedure InsertAmdNsiParts(pNsi_sid in amd_nsi_parts.nsi_sid%type) is
		begin
			insertNsiParts(pNsi_sid => pNsi_sid,
				pPart_no => pPart_no,
		...
		procedure InsertEquivalentPartData( pNsi_sid in amd_nsi_parts.nsi_sid%type) is
		begin
		...
		procedure DoPhysicalInsert is
			...
			procedure PrepareDataForInsert is
			begin
			...
			procedure UpdatePrimePartData( pNsi_sid in amd_national_stock_items.nsi_sid%type) is
			...
			begin -- UpdatePrimePartData
				...
				UpdtNsiPrimePartData (pPrime_ind => pPrime_ind,
	 					  pNsi_sid => pNsi_sid,
	 					  pPartNo => pPart_no,
	 					  pNsn => pNsn,
						  pItem_type => pItem_type,
			...
			exception
				when others then
					...
					ErrorMsg(pSqlfunction => 'updatePrimePartData', pError_location => 320) ;
					raise ;
			end UpdatePrimePartData;


			procedure UpdatePrimePartData( pNsn in amd_national_stock_items.nsn%type,
				pNsi_sid in amd_nsns.nsi_sid%type,
				pCurrent_prime_part_no in amd_nsi_parts.part_no%type) is

				result number := SUCCESS;

				procedure MakePrimeAnEquivalentPart is
		...
		begin -- DoPhysicalInsert
			debugMsg('DoPhysicalInsert', pError_location => 150) ;
			PrepareDataForInsert;

			if NatStkItemExists(pNsn => pNsn, pNsi_sid => DoPhysicalInsert.nsi_sid) then
				null ; -- OK do nothing
			else -- create one
			    CreateNationalStockItem(pNsi_sid => DoPhysicalInsert.nsi_sid,
		 			   	  pNsn => pNsn,
		 				  pItem_type => pItem_type,
		 				  pOrder_quantity => pOrder_quantity,
		 				  pPlanner_code => pPlanner_code,
    ...
    begin -- <<<---- InsertRow
		...
		debugMsg(mArgs, pError_location => 5) ;
...
--		insertLoadDetail(pPart_No,pNsn,pPrime_Ind,'Insert');
		...
		if is_spo_part = 'N' and a2a_pkg.isPartActive(pPart_no) then
		...
		if is_spo_part = 'Y' then
		   declare
			...
			if rc = SUCCESS then
					    ...
					    rc := a2a_pkg.InsertPartLeadTime(pPart_no,a2a_pkg.NEW_BUY,
						amd_preferred_pkg.GetPreferredValue( mOrderLeadTimeCleaned, 
							pOrder_lead_time, amd_defaults.GetOrderLeadTime(pItem_type)) );
                        		    a2a_pkg.insertPartLeadTime(part_no => pPart_no, 
                            			lead_time_type => a2a_pkg.REPAIR,
                            			lead_time => amd_preferred_pkg.GetPreferredValue(mTimeToRepairOffBaseCleand,
						       	mTimeToRepairOffBase, amd_defaults.TIME_TO_REPAIR_OFFBASE), 
                            			action_code => amd_defaults.INSERT_ACTION); 
                    		else
				    rc := a2a_pkg.InsertPartLeadTime(pPart_no, a2a_pkg.NEW_BUY,
						amd_preferred_pkg.GetPreferredValue( pOrder_lead_time, 
							amd_defaults.GetOrderLeadTime(pItem_type)) );
                    		end if ;                        
			end if ;
			if rc = SUCCESS then
			   result := a2a_pkg.InsertPartPricing(pPart_no,a2a_pkg.AN_ORDER,
				amd_preferred_pkg.GetPreferredValue(mUnitCostCleaned,pUnit_cost)) ; -- used for SCM 4.2
			end if ;
	...
	function UpdateRow(
		...
		*/
		...
		procedure PrepareDataForUpdate is
			function GetSmrCode
		...
		procedure UpdateAmdSparePartRow( pPartNo amd_spare_parts.part_no%type,
				pNsn amd_spare_parts.nsn%type) is
		begin
			debugMsg('updateAmdSparePartRow('||pPartNo||','||pNsn||')', pError_location => 160);
		...
		exception when others then
			...
			ErrorMsg(pSqlfunction => 'updateAmdSparePartRow', pError_location => 420 ) ;
			raise ;
		end UpdateAmdSparePartRow;


		procedure UpdatePrimePartData is
		begin
			<<update_amd_nsns>>
			begin
				amd_spare_parts_pkg.UpdateAmdNsn(
					   pNsn_type => pNsn_type,
					   pNsi_sid => nsiSid,
					   pNsn => pNsn);
			exception when others then
		...
		function NsnChanged( pPartNo varchar2, pNsn varchar2) return boolean is
			nsn amd_nsns.nsn%type;
		begin
			debugMsg('nsnChanged('||pPartNo||','||pNsn||')', pError_location => 170);
			select an.nsn
			into nsn
			from
				amd_nsi_parts anp,
		...
		function PrimeIndChanged return boolean is
			prime_ind amd_nsi_parts.prime_ind%type := null;
		begin
			debugMsg('primeIndChanged(' || prime_ind || ')', pError_location => 180);
		...
		*/
			...
			procedure UpdtNsnOfNationalStockItems(pNsiSid number) is
			begin
				debugMsg('updtNsnOfNationalStockItems('||pNsn||','||pNsiSid||')', pError_location => 190);
				update amd_national_stock_items set
					nsn = pNsn
		...
		begin -- UpdateNsnForPrimePart
			...
			MakeNsnSameForAllParts(pNsi_sid => nsiSid,
					   	  						   pNsn => pNsn);
			return result;
		exception when others then
		...
		procedure UpdatePrimeInd is

			procedure UnassignPrimePart(pPart_no in amd_nsi_parts.part_no%type) is
			begin
				debugMsg('unassignPrimePart(' || pPart_no || ')', pError_location => 200);
			...
			procedure MakeCurrentPrimeIntoEquiv is
			...
			begin
				...
				insertNsiParts(pNsi_sid => nsiSid,
					pPart_no => part_no,
					pPrime_ind => amd_defaults.NOT_PRIME_PART,
					pPrime_ind_cleaned => null,
			...
			procedure UpdatePrimePartNo is
		...
		begin --  UpdatePrimeInd
			debugMsg('updatePrimeInd()', pError_location => 210);
			if IsPrimePart(pPrime_ind) then
		...
		procedure InsertNewNsn( pNsi_sid out amd_nsns.nsi_sid%type) is

			/* Get the nsi_sid using the part_no */
			procedure GetNsiSid is
			begin
			...
			exception
				...
				when others then
					...
					ErrorMsg(pSqlfunction => 'getNsiSid', pError_location => 540) ;
					raise ;
			end GetNsiSid;
		...
		exception
			...
			when others then
				...
				ErrorMsg(pSqlfunction => 'insertNewNsn', pError_location => 550) ;
				raise ;
		end InsertNewNsn;


		function GetNsiSid return amd_nsns.nsi_sid%type is
			nsi_sid amd_nsns.nsi_sid%type ;
		begin
			debugMsg('getNsiSid()', pError_location => 220);
			nsi_sid := amd_utils.GetNsiSid(pNsn => pNsn);
			debugMsg('Nsi_sid=' || Nsi_sid, pError_location => 230) ;
			return nsi_sid ;
		exception
			when no_data_found then
			...
			when others then
				ErrorMsg(pSqlfunction => 'getNsiSid', pError_location => 560) ;
				raise ;
		end GetNsiSid;


		procedure CheckNsnAndPrimeInd is
			result number := SUCCESS;
		begin
			debugMsg('checkNsnAndPrimeInd()',pError_location => 240);
		...
		exception
			when amd_spare_parts_pkg.CANNOT_FIND_PART then
				ErrorMsg(pSqlfunction => 'CheckNsnAndPrimeInd', pError_location => 570) ;
				raise ;
			when others then
				mRC := amd_spare_parts_pkg.CHK_NSN_AND_PRIME_ERR2 ;
				ErrorMsg(pSqlfunction => 'CheckNsnAndPrimeInd', pError_location => 580) ;
				raise ;
		...
		begin
			 ...
			 or amd_utils.isDiff(order_lead_time_cleaned,mOrderLeadTimeCleaned) then
			 	result := a2a_pkg.UpdatePartLeadTime(pPart_no,a2a_pkg.NEW_BUY,
					amd_preferred_pkg.GetPreferredValue(mOrderLeadTimeCleaned, 
						pOrder_lead_time, amd_defaults.GetOrderLeadTime(pItem_type))) ;
			 end if ;
		...
		begin
			 ...
			 or amd_utils.isDiff(unit_cost_cleaned, pUnit_cost) then
			 	result := a2a_pkg.UpdatePartPricing(pPart_no,a2a_pkg.AN_ORDER,
						amd_preferred_pkg.GetPreferredValue(mUnitCostCleaned, pUnit_Cost)) ;
			 end if ;
			 return result ;
		exception
			when standard.no_data_found then
		...
		begin
			...
			*/
		exception when others then
			ErrorMsg(pSqlfunction => 'validateInput', pError_location => 610) ;
			raise ;
		end validateInput ;
		
	begin -- <<<---- UpdateRow
		validateInput ;
		debugMsg(mArgs  || ')',pError_location => 250);
--		insertLoadDetail(pPart_No,pNsn,pPrime_Ind,'Update');
		...
		--
		lineNumber := 100 ;
		debugMsg('updating action code to D',pError_location => 260) ;
		update amd_national_stock_items set
			action_code    = 'D',
			last_update_dt = sysdate
		where
		action_code != 'D'
		and nsi_sid in (select nsi_sid
				from amd_nsi_parts
				where part_no = pPart_no
				and nsi_sid != nsiSid
				minus
				...
				from amd_nsi_parts
				where
				nsi_sid in (select nsi_sid from amd_nsi_parts
					    where part_no = pPart_no)
					    and unassignment_date is null);
		...
		lineNumber := 110 ;
		if result = SUCCESS and is_spo_part = 'Y'  then
			<<processA2A>>
			declare
				rc number ;
				smrCodePreferred amd_national_stock_items.SMR_CODE%type 
	...
	exception
		when others then
			ErrorMsg(pSqlfunction => 'updateRow', pError_location => 630, pKey_1  => lineNumber) ;
			return mRC ;
	end UpdateRow;
	...
	function DeleteRow( pPart_no in varchar2,
		...
		is_spo_part amd_spare_parts.is_spo_part%type ;
		was_part_sent boolean := a2a_pkg.wasPartSent(pPart_no) ;
	...
	begin
		debugMsg('amd_spare_parts.deleteRow: ' || pPart_no, pError_location => 631) ;
                
		mArgs := 'DeleteRow(' || pPart_no || ', ' || pMfgr || ', ' || pNomenclature || ')' ;
               
		if amd_utils.ISSPOPRIMEPART(pPart_no) and was_part_sent then
			/* delete the REPAIR */
			a2a_pkg.insertPartLeadTime(part_no => pPart_no,
				lead_time_type => a2a_pkg.REPAIR,
				lead_time => 0,
		...
		debugMsg('amd_spare_parts.deleteRow: ' || pPart_no, pError_location => 632) ;
		return result ;

	exception when others then
		ErrorMsg(pSqlfunction => 'deleteRow', pError_location => 650) ;
		return mRC ;
	end DeleteRow ;
	...
	procedure loadCurrentBackOrder(debug in boolean := False) is
		...
		where prime_part_no not in 
			...
			and req1.select_from_sc like 'C17%' 
			and req1.status in ('U','H','O','R') 
	...
	begin
		 writeMsg(pTableName => 'amd_spare_parts_pkg', 
		 		pError_location => 740, pKey1 => 'amd_spare_parts_pkg', pKey2 => '$Revision:   1.111  $') ;
		 dbms_output.put_line('amd_spare_parts_pkg: $Revision:   1.111  $') ;
	end version ;
	...
	begin
		mDebug := upper(switch) in ('Y','T','YES','TRUE') ;
		if mDebug then
