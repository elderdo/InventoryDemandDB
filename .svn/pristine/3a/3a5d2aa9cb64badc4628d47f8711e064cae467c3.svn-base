...
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.AMD_LOAD AS
   ...
   added infoMsg, which is almost the same as ErrorMsg, but it does not do a Rollback or a commit.
    ...
    exception when others then
        ErrorMsg(sqlFunction => 'getCleanedBssmData',tableName => 'bssm_parts', pError_location => 85,
	...
	BEGIN
        ...
        if catRecs.first is not null then
            FOR indx in catRecs.first .. catRecs.last LOOP
                ...
                --	orderleadTime := Amd_Utils.bizDays2CalendarDays(rec.ave_cap_lead_time);
                ...
                if mdebug and mod(indx,debugThreshold) = 0 then
                    dbms_output.put_line('indx=' || indx || ' loadGold 100: ' || (dbms_utility.get_time - loopStartTime)) ;
                end if ;                    
                ...
                DECLARE
                       ...
                       begin
                           ...
                           recsOut(indx).order_lead_time :=  amd_utils.bizDays2CalendarDays(getOrderLeadTime(catRecs(indx).part)) ;
                           recsOut(indx).prime_ind :=  primeInd ;
                           recsOut(indx).serial_flag :=  catRecs(indx).serial_mandatory_b ;
                           recsOut(indx).smr_code :=  smrCode ;
                           recsOut(indx).acquisition_advice_code :=  catRecs(indx).user_ref7 ;
                           ...
                           recsOut(indx).time_to_repair_off_base_cleand :=  time_to_repair_off_base_cleand ;
                           recsOut(indx).order_lead_time_cleaned :=  order_lead_time_cleaned ;
                           ...
                           recsOut(indx).cost_to_repair_off_base :=  cost_to_repair_off_base ; 
                           recsOut(indx).time_to_repair_off_base :=  time_to_repair_off_base ; 
                ...
                BEGIN
                    ...
                    if mDebug and mod(indx,debugThreshold) = 0 then
                        dbms_output.put_line('indx=' || indx || ' loadGold 105 exec secs:' || (dbms_utility.get_time - loopStartTime)) ;
                    end if ;                    
                    /*
                    ...
                    VALUES
                    (	catRecs(indx).part, catRecs(indx).manuf_cage,
                        catRecs(indx).source_code,
                        itemType,
                        catRecs(indx).noun,			nsnStripped,
                        nsnType,			plannerCode,
                        catRecs(indx).um_cap_code,
                        amd_utils.bizDays2CalendarDays(getOrderLeadTime(catRecs(indx).part)),
                        primeInd, 	catRecs(indx).serial_mandatory_b,
                        smrCode, 	catRecs(indx).user_ref7,
                        unitCost,
                        l67Mic,
                        mMac,
                        unitOfIssue,
                ...
                if mDebug and mod(indx,debugThreshold) = 0 then
                    dbms_output.put_line('indx=' || indx || 'loadGold 110 exec secs:' || (dbms_utility.get_time - loopStartTime)) ;
                end if ;                    
        ...
        dbms_output.put_line('loadGold min:' ||
	...
	BEGIN
		...
		commit ;
        dbms_output.put_line('loadPsms secs:' ||
	...
	PROCEDURE LoadTempNsns IS
        ...
        cnt       NUMBER := 0;
	BEGIN
	    writeMsg(pTableName => 'amd_nsns', pError_location => 171,
		pKey1 => 'loadTempNsns',
		pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
	    ...
	    writeMsg(pTableName => 'amd_nsns', pError_location => 173,
		pKey1 => 'loadTempNsns',
		pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
	...
	BEGIN
		 ...
		 VALUES (trim(bems_id), trim(stable_email), trim(last_name), trim(first_name),  Amd_Defaults.INSERT_ACTION, SYSDATE) ;
 		 A2a_Pkg.insertTmpA2ASpoUsers(trim(bems_id), trim(stable_email), trim(last_name), trim(first_name), Amd_Defaults.INSERT_ACTION) ;
		 RETURN SUCCESS ;
	EXCEPTION
		when standard.dup_val_on_index then
			 doUpdate ;
		...
		Mta_Truncate_Table('tmp_a2a_spo_users','reuse storage');
             ...
             FOR indx in bemsIdRecs.first .. bemsIdRecs.last LOOP
                 IF bemsIdRecs(indx) IS NOT NULL THEN
                   ...
                   BEGIN
                     ...
                     inserted := inserted + 1 ;
                     A2a_Pkg.spoUser(bemsIdRecs(indx),  Amd_Defaults.INSERT_ACTION) ;
         ...
         if bemsIdRecs.first is not null then
             FOR indx in bemsIdRecs.first .. bemsIdRecs.last LOOP
                 ...
                 deleted := deleted + 1 ;
                 A2a_Pkg.spoUser(bemsIdRecs(indx),  Amd_Defaults.DELETE_ACTION) ;
             END LOOP ;
        end if ;             
	...
	begin
		 writeMsg(pTableName => 'amd_load', 
		 		pError_location => 360, pKey1 => 'amd_load', pKey2 => '$Revision:   1.71  $') ;
		 dbms_output.put_line('amd_load: $Revision:   1.71  $') ;
	end version ;
