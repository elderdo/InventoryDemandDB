...
CREATE OR REPLACE PACKAGE AMD_OWNER.USER_PLANNER_PKG AS
 ...
 /*
      $Author:   zf297a  $
    $Revision:   1.1  $
        $Date:   02 Oct 2008 14:24:32  $
    $Workfile:   user_planner_pkg.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 3.3 Implementation\user_planner_pkg.sql.-arc  $
/*   
/*      Rev 1.1   02 Oct 2008 14:24:32   zf297a
/*   Disabled PVCS keyword expansion and added procedure version and function getVersion
/*   
...
*/
   ...
   function insertSiteAssetMgr(bems_id in amd_site_asset_mgr.bems_id%type, 
                planner_code in amd_site_asset_mgr.planner_code%type) return number ;

   function deleteSiteAssetMgr(bems_id in amd_site_asset_mgr.bems_id%type, 
                planner_code in amd_site_asset_mgr.planner_code%type) return number ;
...
*/
    ...
    function insertSiteAssetMgr(bems_id in amd_site_asset_mgr.bems_id%type, 
                planner_code in amd_site_asset_mgr.planner_code%type) return number is
    ...
    begin
        ...
        begin
            ...
            writeMsg(pTableName => 'amd_site_asset_mgr', pError_location => 55,
                pKey1 => bems_id,
                pKey2 => planner_code,
                pKey3 => 'bems_id already exists in amd_users',
                pKey4 => 'amd_users will be sued for x_user_v and spo_user') ;            
        exception when standard.no_data_found then
            user_exists := 0 ;
        end checkAmdUsers ;
        
        if not isValidPlannerCode(planner_code) then
             ErrorMsg(pTableName => 'amd_site_asset_mgr',
				   pError_location => 60,
				   pKey1 => bems_id,
                   pKey2 => planner_code,
                   pKey3 => 'invalid_planner_code') ;
            dbms_output.put_line('planner_code: ' || bems_id || ' is invalid.') ;                   
            return INVALID_PLANNER_CODE ;                   
        end if ;
        insert into amd_site_asset_mgr
        (bems_id, planner_code)
        values(insertSiteAssetMgr.bems_id, insertSiteAssetMgr.planner_code) ;        
        return SUCCESS ;                               
    exception
        when standard.no_data_found then
            return sqlcode ;
    ...
    begin
        ...
        and planner_code = deleteSiteAssetMgr.planner_code ;
        return SUCCESS ;
    exception 
        when standard.no_data_found then
            ErrorMsg(pTableName => 'amd_site_asset_mgr',
