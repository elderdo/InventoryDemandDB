...
/* Formatted on 2008/09/24 14:19 (Formatter Plus v4.8.8) */
...
AS
   SELECT part, parts2.spo_prime_part_no planned_part, supersession_type,
     ...
     FROM (SELECT amd_utils.getprimepartno (old_nsn) part,
                  amd_utils.getprimepartno (planned_nsn) planned_part,
                  supersession_type
             FROM (SELECT old_nsn, new_nsn, 'TWO-WAY' supersession_type,
                          subgroup_code, part_pref_code
                     FROM amd_rbl_pairs a
                    WHERE old_nsn <> new_nsn
                      AND subgroup_code = (SELECT subgroup_code
                                             FROM amd_rbl_pairs
                                            WHERE old_nsn = a.new_nsn)) twoway,
