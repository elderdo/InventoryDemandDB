/* Formatted on 1/25/2017 6:10:27 PM (QP5 v5.287) */
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.AMD_LOCATION_PART_LEADTIME_PKG
AS
   /*
   ...
   /*      Rev 1.24 25 Jan 2017 DSE reformatted text added dbms_output of rows inserted

   /*
   /*      Rev 1.23   5 Jun 2015   zf297a
   /*   could not use amd_defaults.getLocId for getRampData because of pragma
   /*
   ...
   */
   ...
   CURSOR locPartLeadtime_cur
   IS
      SELECT spo_prime_part_no,
             loc_sid,
             DECODE (NVL (GetAvgRepairCycleTime (parts.nsn, loc_id), 0),
                     0, Amd_Defaults.TIME_TO_REPAIR_ONBASE,
                     GetAvgRepairCycleTime (parts.nsn, loc_id))
                time_to_repair,
   ...
   FUNCTION IsPartRepairable (pPartNo amd_spare_parts.part_no%TYPE)
      RETURN VARCHAR2
   IS
   BEGIN
   ...
   FUNCTION IsPartRepairable (pNsiSid amd_national_stock_items.nsi_sid%TYPE)
      RETURN VARCHAR2
   IS
   ...
   BEGIN
      ...
      IF (ansiRow.smr_code_cleaned IS NOT NULL)
      THEN
         smr := ansiRow.smr_code_cleaned;
      ELSIF (ansiRow.smr_code IS NOT NULL)
      THEN
   ...
   IS
   BEGIN
      Amd_Utils.writeMsg (pSourceName       => 'amd_location_part_leadtime_pkg',
   ...
   FUNCTION GetRampData (pNsn       amd_nsns.nsn%TYPE,
   ...
   IS
      ...
      IS
         SELECT *
           FROM ramp
          WHERE     REPLACE (current_stock_number, '-') = pNsn
                AND SUBSTR (sc, 8, 6) = pLocId;
   ...
   FUNCTION GetAvgRepairCycleTime (pNsn      amd_nsns.nsn%TYPE,
                                   pLocId    amd_spare_networks.loc_id%TYPE)
      RETURN ramp.AVG_REPAIR_CYCLE_TIME%TYPE
   IS
   ...
   PROCEDURE UpdateAmdLocPartLeadtime (
      ...
      pLastUpdateDt    amd_location_part_leadtime.last_update_dt%TYPE)
   IS
      returnCode   NUMBER;
   BEGIN
      UPDATE amd_location_part_leadtime
         SET time_to_repair = pTimeToRepair,
   ...
   IS
   BEGIN
      INSERT INTO amd_location_part_leadtime (part_no,
                                              loc_sid,
                                              time_to_repair,
                                              action_code,
                                              last_update_dt)
   ...
   EXCEPTION
      ...
      THEN
         UpdateAmdLocPartLeadtime (pPartNo,
                                   pLocSid,
                                   pTimeToRepair,
                                   pActionCode,
                                   SYSDATE);
   ...
   */
   ...
   BEGIN
      BEGIN
         InsertAmdLocPartLeadtime (pPartNo,
                                   pLocSid,
                                   pTimeToRepair,
   ...
   BEGIN
      ...
      IF locPartLeadTimeRecs.FIRST IS NOT NULL
      THEN
         ...
         DBMS_OUTPUT.put_line (
            'LoadTmpAmdLocPartLeadtime: rows inserted ' || locPartLeadTimeRecs.LAST);
         COMMIT;
      END IF;
   EXCEPTION
      WHEN OTHERS
      THEN
         ErrorMsg (pSqlFunction      => 'forall insert',
   ...
   BEGIN
      writeMsg (pTableName        => 'amd_location_part_leadtime_pkg',
                pError_location   => 180,
                pKey1             => 'amd_location_part_leadtime_pkg',
                pKey2             => '$Revision:   1.23  $');
      DBMS_OUTPUT.put_line (
         'amd_location_part_leadtime_pkg: $Revision:   1.23  $');
   END version;
   ...
   FUNCTION getVIRTUAL_COD_SPO_LOCATION
   ...
   FUNCTION getVIRTUAL_UAB_SPO_LOCATION
      RETURN amd_spare_networks.spo_location%TYPE
   IS
   BEGIN
