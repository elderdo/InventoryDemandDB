...
/*
       ...
       $Author:   zf297a  $
     $Revision:   1.77.1  $
         $Date:   19 Mar 2010 08:39  $
     $Workfile:   amd_load.pkb  $
	      $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Packages\amd_load.pkb-arc  $
   
      Rev 1.77.1    19 Mar 2010 Made changes to function getOffBaseTurnAround to comply with ClearQuest LBPSS00002451 - ClearCase branch getOffBaseTurnAroundPatch
   ...
   added infoMsg, which is almost the same as ErrorMsg, but it does not do a Rollback or a commit.
    ...
    begin
         writeMsg(pTableName => 'amd_load', 
                 pError_location => 630, pKey1 => 'amd_load', pKey2 => '$Revision:   1.77.1  $') ;
         dbms_output.put_line('amd_load: $Revision:   1.77.1  $') ;
    end version ;
    
    function getVersion return varchar2 is
    begin
        return '$Revision:   1.77.1  $' ;
    end getVersion ;
