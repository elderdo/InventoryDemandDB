...
/*   Initial revision.
	...
	TSL_OVERRIDE_TYPE 	  	 CONSTANT tmp_a2a_loc_part_override.OVERRIDE_TYPE%type := 'TSL Fixed' ;
...
/*   Initial revision.
	...
	PROCEDURE InsertTmpAmdLocPartOverride (
			  ...
			  pLastUpdateDt				AMD_LOCATION_PART_OVERRIDE.last_update_dt%TYPE) IS
	BEGIN
        if pTslOverrideQty <> 0 and pActionCode <> amd_defaults.INSERT_ACTION  then -- Xzero
	...
	FUNCTION insertedTmpA2ALPO (
			...
			begin
                    ...
                    if (pOverrideType = TSL_OVERRIDE_TYPE and not amd_utils.isPartRepairable(pPartNo))
                    ...
                    or (pTslOverrideQty = 0 and pOverrideType = TSL_OVERRIDE_TYPE 
                        and action_code = amd_defaults.INSERT_ACTION) then -- Xzero
                        return ;
                    end if ;                        
        ...
        */
		...
		if reqRecs.first is not null then
            forall indx in reqRecs.first .. reqRecs.last 
                insert into tmp_amd_location_part_override values reqRecs(indx) ;
        /*                   	
                begin
                    update tmp_amd_location_part_override
		...
		begin
			...
			commit ;
			   ...
			   and not exists (
                     ...
                     and override_type = amd_location_part_override_pkg.TSL_OVERRIDE_TYPE) 
        ...
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 940,
         ...
         OPEN dataByDate FOR
              SELECT alpo.part_no,
                    ...
                    TSL_OVERRIDE_TYPE AS override_type,
    ...
    PROCEDURE LoadAllA2A( useTestData IN BOOLEAN := FALSE, from_dt IN DATE := A2a_Pkg.start_dt, to_dt IN DATE := SYSDATE) IS
             ...
             OPEN overrides FOR
                ...
                case 
                     when rsp.action_code = amd_defaults.getDELETE_ACTION or sent.action_code = amd_defaults.getDELETE_ACTION then
                           0
                     when override_type <> amd_location_part_override_pkg.TSL_OVERRIDE_TYPE and rsp_level > 0 then 
                           rsp_level - 1                           
                     else
                          rsp_level
        ...
        begin
            ...
            rsp_location,
                override_type,
                case 
                     when rsp_repairables.action_code = amd_defaults.getDELETE_ACTION or sent.action_code = amd_defaults.getDELETE_ACTION then
                           0
                     else
                          rsp_level
                end override_quantity,
            amd_location_part_override_pkg.getOVERRIDE_REASON as override_reason,
            Amd_Location_Part_Override_Pkg.GetFirstLogonIdForPart(Amd_Utils.GetNsiSidFromPartNo(rsp_repairables.part_no)),
            sysdate as begin_date,
            null as end_date,
            case sent.action_code
                 when amd_defaults.getDELETE_ACTION then
                      ...
                      case rsp_repairables.action_code
                         when amd_defaults.getDELETE_ACTION then
                               amd_defaults.getUPDATE_ACTION -- the part is still valid, update the current value to zero
                         else
                                rsp_repairables.action_code
                    end                    
            end AS action_code,
            sysdate as last_update_dt
            from amd_rsp_sum_repairables_v rsp_repairables, 
            amd_sent_to_a2a sent,
            amd_spare_networks nwks
            where trunc(rsp_repairables.last_update_dt) between trunc(from_dt) and trunc(to_dt) 
       	    and rsp_repairables.part_no = sent.part_no
            ...
            rsp_location,
                override_type,
                case 
                     when rsp_consumables.action_code = amd_defaults.getDELETE_ACTION or sent.action_code = amd_defaults.getDELETE_ACTION then
                           0
                     else
                          rop_or_roq
                end override_quantity,
            amd_location_part_override_pkg.getOVERRIDE_REASON as override_reason,
            Amd_Location_Part_Override_Pkg.GetFirstLogonIdForPart(Amd_Utils.GetNsiSidFromPartNo(rsp_consumables.part_no)),
            sysdate as begin_date,
            null as end_date,
            case sent.action_code
                 when amd_defaults.getDELETE_ACTION then
                      ...
                      case rsp_consumables.action_code
                         when amd_defaults.getDELETE_ACTION then
                               amd_defaults.getUPDATE_ACTION -- the part is still valid, update the current value to zero
                         else
                                rsp_consumables.action_code
                    end                    
            end AS action_code,
            sysdate as last_update_dt
            from amd_rsp_sum_consumables_v rsp_consumables, 
            amd_sent_to_a2a sent,
            amd_spare_networks nwks
            where trunc(rsp_consumables.last_update_dt) between trunc(from_dt) and trunc(to_dt) 
       	    and rsp_consumables.part_no = sent.part_no
            and sent.spo_prime_part_no = sent.part_no 
            and sent.SPO_PRIME_PART_NO is not null
            and substr(rsp_location,1,length(rsp_location) - 4) = nwks.mob
            and nwks.mob is not null ;
        ...
        begin
            ...
            amd_sent_to_a2a sent,
            amd_spare_networks nwks
            where  rsp_repairables.part_no = sent.part_no
            and sent.spo_prime_part_no = sent.part_no 
            and sent.SPO_PRIME_PART_NO is not null
            and substr(rsp_location,1,length(rsp_location) - 4) = nwks.mob
            and nwks.mob is not null  
            ...
            amd_sent_to_a2a sent,
            amd_spare_networks nwks
            where  rsp_consumables.part_no = sent.part_no
            and sent.spo_prime_part_no = sent.part_no 
            and sent.SPO_PRIME_PART_NO is not null
            and substr(rsp_location,1,length(rsp_location) - 4) = nwks.mob
            and nwks.mob is not null ; 
             ...
             OPEN tsl FOR
                ...
                FROM (
                    ...
                    AND not exists (
                             ...
                             and override_type = amd_location_part_override_pkg.TSL_OVERRIDE_TYPE)                               
                    AND sent.spo_prime_part_no IN (SELECT part_no FROM AMD_TEST_PARTS)
    ...
    begin
         writeMsg(pTableName => 'amd_location_part_override_pkg', 
                 pError_location => 1360, pKey1 => 'amd_location_part_override_pkg', pKey2 => '$Revision:   1.96  $') ;
         dbms_output.put_line('amd_location_part_override_pkg: $Revision:   1.96  $') ;
    end version ;
