...
*/
            ...
            loop
                ...
                if (whseBaseRecs(indx).rop + whseBaseRecs(indx).roq <> 0) or loadZeroTsls then
                    ...
                    if isTestPart(whseBaseRecs(indx).spo_prime_part_no) then
                        debugMsg(whseBaseRecs(indx).spo_prime_part_no || ' rop + roq = 0', pError_location => 80) ;
                    end if ;                        
                end if ;                                     
                    --recs => recs) ;
            end loop ;
            ...
            for indx in lvlsRecs.first .. lvlsRecs.last loop
                ...
                if (lvlsRecs(indx).roq + lvlsRecs(indx).rop <> 0) or loadZeroTsls then
                    ...
                    if isTestPart(lvlsRecs(indx).spo_prime_part_no) then
                        debugMsg(lvlsRecs(indx).spo_prime_part_no || ' rop + roq = 0', pError_location => 110) ;
                    end if ;                        
                end if ;                        
            end loop ;
        end if ;
    ...
    procedure loadWhse  is
                            ...
                            when nvl(a.approved_lvl_qty,0) > 0 then
                                case when a.approved_lvl_qty - nvl(a.reorder_point,0) = 1
                                    and nvl(reorder_point,0) <= 0 then
                                        0
                                     else round(a.approved_lvl_qty - nvl(a.reorder_point,0))
                                end
                            else 0
                        end
                 ) roq
        ...
        begin
            debugMsg('sum the recs',  pError_location => 130) ;
            if recs.first is not null then
                for indx in recs.first .. recs.last loop
                    ...
                    if isTestPart(recs(indx).spo_prime_part_no) then
                        ...
                        || ' base roq=' || recs(indx).roq , pError_location => 140) ;
                        debugMsg(recs(indx).spo_prime_part_no || ': rop= ' || rop
                        || ' roq=' || roq, pError_location => 150) ;
                    end if ;
        ...
        begin
            ...
            begin
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    debugMsg(rec.spo_prime_part_no ||': whse_rop=' || whse_rop || ' whse_roq=' || whse_roq || ' rec.rop= ' || rec.rop || ' rec.roq=' || rec.roq, pError_location => 160) ;
                end if ;
                if recFound then
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    debugMsg(rec.spo_prime_part_no  ||': rop= ' || rop || ' *** roq=' || roq || ' ***', pError_location => 170) ;
                end if ;
            ...
            */
            ...
            if (rop + roq = 0)  then
                if isTestPart(rec.spo_prime_part_no) then
                    debugMsg(rec.spo_prime_part_no || ' rop + roq = 0 ** part not sent to spo **', pError_location => 180) ;
                end if ;
            else
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    debugMsg(rec.spo_prime_part_no ||  ' accepted', pError_location => 190) ;
                end if ;
            end if ;
        exception when others then
                 ...
                 sum(
                    case when nvl(whse.reorder_point,0) > 0 then
                            case when to_number(nvl(whse.reorder_point,0)) = 1
                                and ' || roq || ' <= 0 then
                                    0
                                else round(to_number(nvl(whse.reorder_point,0)))
                            end
                         else
                            0
                    end)  rop,
   ...
   begin
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 220,
                pKey1 => 'loadWhse',
                pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM')) ;

        debugMsg('process wesmParts', pError_location => 230) ;
        open wesmParts ;
        fetch wesmParts bulk collect into wesmRecs ;
        close wesmParts ;
        sumTheRecs(recs => wesmRecs, sums => wesmSumRecs) ;

        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 240,
                pKey1 => 'loadWhse',
                pKey2 => 'at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
                pKey3 => 'wesmRecs.count=' || to_char(wesmRecs.count)) ;


        debugMsg('process wesm parts whsePart for UK', pError_location => 250) ;
        ...
        debugMsg('process wesm parts whsePart for BASC', pError_location => 270) ;
        ...
        debugMsg('process wesm parts whsePart for AUS', pError_location => 272) ;
        ...
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 274,
                pKey1 => 'loadWhse',
                pKey2 => 'at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
                pKey3 => 'processed wesm AUS') ;


        debugMsg('process wesmSumRecs', pError_location => 290) ;
        partIndx := wesmSumRecs.first ;
        while partIndx is not null loop
            if isTestPart(wesmSumRecs(partIndx).spo_prime_part_no) then
                debugMsg(wesmSumRecs(partIndx).spo_prime_part_no || ': rop= ' || wesmSumRecs(partIndx).rop || ' roq=' || wesmSumRecs(partIndx).roq, pError_location => 300) ;
            end if ;
        ...
        debugMsg('wesmSumRecs.count=' || wesmSumRecs.count, pError_location => 320) ;

        debugMsg('process lvls_by_base', pError_location => 330) ;
        open lvls_by_base ;
        ...
        debugMsg('process non-wesm parts whsePart for UK', pError_location => 350) ;
        processWhseParts(spo_location => amd_defaults.AMD_UK_LOC_ID,
                            sc => amd_defaults.AMD_UK_SC,
        ...
        debugMsg('process non-wesm parts whsePart for BASC', pError_location => 370) ;
        processWhseParts(spo_location => amd_defaults.AMD_BASC_LOC_ID,
                            sc => amd_defaults.AMD_BASC_SC,
        ...
        debugMsg('process non-wesm parts whsePart for AUS', pError_location => 372) ;
        processWhseParts(spo_location => amd_defaults.AMD_AUS_LOC_ID,
                            sc => amd_defaults.AMD_AUS_SC,
                            operator => 'like', wesm_part =>'N',
                            sumRecs => nonwesmSumRecs,
                            roq => 'nvl(whse.STOCK_LEVEL,0) - nvl(whse.reorder_point,0)') ;
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 374,
                pKey1 => 'loadWhse',
                pKey2 => 'at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
        ...
        if recs.first is not null then
            forall indx in recs.first .. recs.last
                ...
                values recs(indx) ;
            commit ;
            debugMsg(recs.count || ' recs loaded', pError_location => 390) ;
        else
            debugMsg('no recs loaded via bulk insert', pError_location => 400) ;
        end if ;
    ...
    begin
                ...
                if (wesmRecs(indx).roq + wesmRecs(indx).rop <> 0) or loadZeroTsls then
                   ...
                   doInsert(part_no => wesmRecs(indx).spo_prime_part_no, spo_location => wesmRecs(indx).spo_location, loc_sid => wesmRecs(indx).loc_sid,
                    ...
                    if isTestPart(wesmRecs(indx).spo_prime_part_no) then
                        debugMsg(wesmRecs(indx).spo_prime_part_no || ' rop + roq = 0', pError_location => 430) ;
                    end if ;                                        
                end if ;                        
     ...
     procedure loadVirtualLocations is
        ...
        tsl_override_user tmp_locpart_overid_consumables.TSL_OVERRIDE_USER%type ;
     begin
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 450,
    ...
    begin
        ...
        /*
        writeMsg(pTableName => 'doLPOverrideConsumablesDiff', pError_location => 490,
           pKey1 => 'doUpdate.part_no=' || part_no,
           pKey2 => 'spo_location=' || spo_location,
           pKey3 => 'type=' || tsl_override_type,
        ...
        if action_code = 'A' then
            ...
            exception when standard.DUP_VAL_ON_INDEX then
                if not loggedDup then
                    writeMsg(pTableName => 'tmp_locpart_overid_consumables', pError_location => 500,
                        pKey1 => 'part_no=' || part_no,
                        pKey2 => 'spo_location=' || spo_location,
                        pKey3 => 'tsl_override_type=' || tsl_override_type,
    ...
    begin
     ...
     begin
        ...
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 530,
            pKey1 => 'loadLocPartOverrides',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM')) ;
        commit ;
    ...
    begin
        writeMsg(pTableName => 'amd_lp_override_consumabl_pkg',
             pError_location => 560, pKey1 => 'amd_lp_override_consumabl_pkg', pKey2 => '$Revision:   1.62  $') ;
        dbms_output.put_line('amd_lp_override_consumabl_pkg: $Revision:   1.62  $') ;
    end version ;
