...
/*   Initial revision.
	...
	FUNCTION insertedTmpA2ALPO (
		...
		begin
			...
			SET 	override_quantity	= pTslOverrideQty,
		...
		exception
			when others then
				ErrorMsg(pSqlfunction => 'doUpdate',
					...
					pKey3  => 'insertedTmpA2ALPO.action_code='
		...
		procedure insertRow is
			procedure recordInfo is
				...
				procedure changed (field in varchar2,
					curfieldValue in varchar2, prevFieldValue in varchar2,
			...
			begin
				select override_type, override_quantity,
					override_reason, override_user,
					begin_date, action_code, last_update_dt
				into override_type, override_quantity,
					override_reason, override_user,
					begin_date, action_code, last_update_dt
				...
				if pTslOverrideQty <> override_quantity then
					changed('override_quantity',
						to_char(pTslOverrideQty),
						to_char(override_quantity),
						pError_location => 70) ;
				end if ;

				if pOverrideReason <> override_reason then
					changed('override_reason',
						pOverrideReason,
						override_reason,
						pError_location => 80) ;
				end if ;
				...
				if pBeginDate <> begin_date then
					changed('begin_date',
						to_char(pBeginDate,'MM/DD/YYYY HH:MI:SS'),
						to_char(begin_date,'MM/DD/YYYY HH:MI:SS'),
						pError_location => 100) ;
				end if ;
				...
				if pLastUpdateDt <> last_update_dt then
					changed('last_update_dt',
						to_char(pLastUpdateDt,'MM/DD/YYYY HH:MI:SS'),
						to_char(last_update_dt,'MM/DD/YYYY HH:MI:SS'),
						pError_location => 120) ;
				end if ;
			exception
				when others then
					ErrorMsg(pSqlfunction => 'recordInfo',
						pTableName => 'tmp_a2a_loc_part_override',
						pError_location => 130,
						pKey1 => 'pPartNo=' || pPartNo,
						pKey2 => 'pBaseName=' || pBaseName,
		...
		begin
			...
			if (pOverrideType = TSL_OVERRIDE_TYPE
				and not amd_utils.isPartRepairable(pPartNo))

			or (pOverrideType in (amd_lp_override_consumabl_pkg.ROP_TYPE,
						amd_lp_override_consumabl_pkg.ROQ_TYPE)
				and not amd_utils.isPartConsumable(pPartNo))

			or pTslOverrideQty is null
			or (pTslOverrideQty = 0
				and pOverrideType = TSL_OVERRIDE_TYPE
			...
			if pTslOverrideQty > 0 then
				INSERT INTO TMP_A2A_LOC_PART_OVERRIDE (
					...
					override_type, override_quantity,
					override_reason, override_user,
					begin_date, action_code,
					last_update_dt)
				VALUES (pPartNo, pBaseName,
					pOverrideType, pTslOverrideQty,
					pOverrideReason, pTslOverrideUser,
					pBeginDate, insertedTmpA2ALPO.action_code,
					pLastUpdateDt) ;
	...
	begin
		...
		if a2a_pkg.isSpoPrimePartActive(pPartNo) then
			...
			action_code := amd_defaults.DELETE_ACTION ;  	-- the part is not active therefore
								-- all associated data must be deleted
		end if ;
		...
		or action_code = amd_defaults.UPDATE_ACTION then
			...
			AND pBaseName IS NOT NULL  THEN
			...
			if a2a_pkg.isPartSent(pPartNo) then
				if ignoreStLouis
				or a2a_pkg.lpOverrideExists(pPartNo, pBaseName) then

					insertRow ; 	-- if the part/base is NOT deleted
							-- in data systems then make sure it gets deleted
				end if ;
			end if ;
	...
	procedure loadOverrideUsers IS
		...
		-- TYPE partNo_logonId_tab IS TABLE OF amd_planner_logons.logon_id%TYPE
			-- INDEX BY amd_spare_parts.part_no%TYPE  ;
		-- partNo_logonId partNo_logonId_tab ;
		-- rowPartNo amd_spare_parts.part_no%TYPE  ;
		-- defaultUser amd_location_part_override.tsl_override_user%TYPE :=
		-- Amd_Defaults.GetParamValue('override_user_default');
    ...
    begin
         writeMsg(pTableName => 'amd_location_part_override_pkg',
                 pError_location => 1360, pKey1 => 'amd_location_part_override_pkg', pKey2 => '$Revision:   1.101  $') ;
         dbms_output.put_line('amd_location_part_override_pkg: $Revision:   1.101  $') ;
    end version ;
