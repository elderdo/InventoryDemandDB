...
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.amd_spare_parts_pkg as
	...
	/*
     ...
     $Workfile:   amd_spare_parts_pkg.pkb  $
      ...
      Rev 1.113   24 Mar 2009 22:31:46   zf297a
   Updated pError_locaton sequences and added debug code
   
      Rev 1.112   24 Feb 2009 14:21:06   zf297a
   Removed a2a code
   ...
   Added $Log$ keyword
    ...
    is_consumable amd_spare_parts.is_consumable%type  ;
    is_repairable amd_spare_parts.is_repairable%type  ;
	mRc		   		  		   	  number := FAILURE ;
	mArgs	   		  		      varchar2(2000) ;
   ...
   procedure updateFlags(pPart_no in amd_spare_parts.part_no%type, is_spo_part out amd_spare_parts.is_spo_part%type) is
	...
	begin
		debugMsg('insertNsiParts('||pNsi_sid||','||pPart_no||','||pPrime_ind||','||pPrime_ind_cleaned||','||pBadRc||')', pError_location => 150);
	...
	begin -- UpdateNatStkItem
	    debugMsg('UpdateNatStkItem(' || pNsn || ', ' || pAction || ', ' || pPartNo || ')', pError_location => 210) ;
	...
	procedure UpdtNsiPrimePartData( pPrime_ind in amd_nsi_parts.prime_ind%type,
		...
		procedure doUpdate(planner_code_cleaned in varchar2, planner_code in varchar2) is
				...
				begin
					 debugMsg('deleteAnyRblPairs', pError_location => 270) ;
		...
		begin
			debugMsg('doUpdate', pError_location => 290) ;
    ...
    begin
        debugMsg('UpdtNsiPrimePartData',pError_location => 310) ;
    ...
    begin -- InsertNatStkItem
        debugMsg('InsertNatStkItem', pError_location => 350) ;
    ...
    begin
        debugMsg('ChgCurNsn2TempNsn(' || pNsiSid || ')', pError_location => 370) ;
        update amd_nsns set
    ...
    begin
        debugMsg('InsertAmdNsn(' || pNsi_sid || ', ' || pNsn || ', ' || pNsn_type, pError_location => 390) ;
    ...
    procedure UpdateAmdNsn(pNsi_sid in amd_nsns.nsi_sid%type,
            ...
            pNsn_type in amd_nsns.nsn_type%type ) is
    begin
        debugMsg('UpdateAmdNsn(' || pNsi_sid || ', ' || pNsn || ', ' || pNsn_type || ')', pError_location => 410) ;
        if pNsn_type = amd_spare_parts_pkg.CURRENT_NSN then
           ChgCurNsn2TempNsn(pNsiSid => pNsi_sid);
        end if ;
    ...
    function InsertRow(
        ...
        */
        ...
        procedure DoPhysicalInsert is
            ...
            exception
                when DUP_VAL_ON_INDEX then       
                    writeMsg(pTableName => 'amd_spare_parts', pError_location => 480,
            ...
            exception
                when others then
                    ...
                    ErrorMsg(pSqlfunction => 'updatePrimePartData', pError_location => 500) ;
                    raise ;
        ...
        begin -- DoPhysicalInsert
            debugMsg('DoPhysicalInsert', pError_location => 540) ;
    ...
    begin -- <<<---- InsertRow
        ...
        debugMsg(mArgs, pError_location => 570) ;
...
--        insertLoadDetail(pPart_No,pNsn,pPrime_Ind,'Insert');
    ...
    function UpdateRow(
        ...
        */
        ...
        begin
            debugMsg('updateAmdSparePartRow('||pPartNo||','||pNsn||')', pError_location => 610);
        ...
        exception when others then
            ...
            ErrorMsg(pSqlfunction => 'updateAmdSparePartRow', pError_location => 620 ) ;
            raise ;
        ...
        function NsnChanged( pPartNo varchar2, pNsn varchar2) return boolean is
            nsn amd_nsns.nsn%type;
        begin
            debugMsg('nsnChanged('||pPartNo||','||pNsn||')', pError_location => 650);
            select an.nsn
            into nsn
            from
                amd_nsi_parts anp,
        ...
        function PrimeIndChanged return boolean is
            prime_ind amd_nsi_parts.prime_ind%type := null;
        begin
            debugMsg('primeIndChanged(' || prime_ind || ')', pError_location => 660);
        ...
        */
            ...
            begin
                debugMsg('updtNsnOfNationalStockItems('||pNsn||','||pNsiSid||')', pError_location => 670);
                update amd_national_stock_items set
                    nsn = pNsn
        ...
        procedure UpdatePrimeInd is
            ...
            begin
                debugMsg('unassignPrimePart(' || pPart_no || ')', pError_location => 700);
        ...
        begin --  UpdatePrimeInd
            debugMsg('updatePrimeInd()', pError_location => 790);
            if IsPrimePart(pPrime_ind) then
        ...
        procedure InsertNewNsn( pNsi_sid out amd_nsns.nsi_sid%type) is
            ...
            exception
                ...
                when others then
                    ...
                    ErrorMsg(pSqlfunction => 'getNsiSid', pError_location => 820) ;
                    raise ;
            end GetNsiSid;
        ...
        exception
            ...
            when others then
                ...
                ErrorMsg(pSqlfunction => 'insertNewNsn', pError_location => 830) ;
                raise ;
        end InsertNewNsn;
        ...
        begin
            debugMsg('getNsiSid()', pError_location => 840);
            nsi_sid := amd_utils.GetNsiSid(pNsn => pNsn);
            debugMsg('Nsi_sid=' || Nsi_sid, pError_location => 850) ;
            return nsi_sid ;
        exception
            when no_data_found then
            ...
            when others then
                ErrorMsg(pSqlfunction => 'getNsiSid', pError_location => 860) ;
                raise ;
        end GetNsiSid;
        ...
        procedure CheckNsnAndPrimeInd is
            result number := SUCCESS;
        begin
            debugMsg('checkNsnAndPrimeInd()',pError_location => 870);
        ...
        exception
            when amd_spare_parts_pkg.CANNOT_FIND_PART then
                ErrorMsg(pSqlfunction => 'CheckNsnAndPrimeInd', pError_location => 880) ;
                raise ;
            when others then
                mRC := amd_spare_parts_pkg.CHK_NSN_AND_PRIME_ERR2 ;
                ErrorMsg(pSqlfunction => 'CheckNsnAndPrimeInd', pError_location => 890) ;
                raise ;
        ...
        begin
            ...
            */
        exception when others then
            ErrorMsg(pSqlfunction => 'validateInput', pError_location => 920) ;
            raise ;
        end validateInput ;
        
    begin -- <<<---- UpdateRow
        validateInput ;
        debugMsg(mArgs  || ')',pError_location => 930);
        
        if amd_utils.isRepairableSmrCode(pSmr_Code) then
            is_repairable := 'Y' ;
            is_consumable := 'N' ;
        else
            is_repairable := 'N' ;
...
--        insertLoadDetail(pPart_No,pNsn,pPrime_Ind,'Update');
        ...
        --
        lineNumber := 100 ;
        debugMsg('updating action code to D',pError_location => 950) ;
        update amd_national_stock_items set
            action_code    = 'D',
            last_update_dt = sysdate
        where
    ...
    exception
        when others then
            ErrorMsg(pSqlfunction => 'updateRow', pError_location => 960, pKey_1  => lineNumber) ;
            return mRC ;
    end UpdateRow;
    ...
    begin
        debugMsg('amd_spare_parts.deleteRow: ' || pPart_no, pError_location => 970) ;
        ...
        exception when others then
            ErrorMsg(pSqlfunction => 'update', pTableName => 'amd_spare_parts',
                pError_location => 980, pKey_1 => pPart_no) ;
            raise ;
        ...
        debugMsg('amd_spare_parts.deleteRow: ' || pPart_no, pError_location => 990) ;
        return result ;

    exception when others then
        ErrorMsg(pSqlfunction => 'deleteRow', pError_location => 1000) ;
        return mRC ;
    end DeleteRow ;
    ...
    begin
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1010,
        ...
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1020,
            pKey1 => 'curDue',
            pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
        ...
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1030,
            pKey1 => 'curDue',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
            pKey3 => 'curDueCnt=' || to_char(curDueCnt) ) ;
        commit ;            
        
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1040,
            pKey1 => 'curTmp1QtyDue',
            pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
        ...
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1050,
            pKey1 => 'curTmp1QtyDue',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
            pKey3 => 'curTmpCnt=' || to_char(curTmpCnt) ) ;
            
        writeMsg(pTableName => 'amd_spare_parts', pError_location => 1060,
            pKey1 => 'loadCurrentBackorder',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
            pKey3 => 'curDueCnt=' || to_char(curDueCnt),
    ...
    begin
         writeMsg(pTableName => 'amd_spare_parts_pkg', 
                 pError_location => 1070, pKey1 => 'amd_spare_parts_pkg', pKey2 => '$Revision:   1.113  $') ;
         dbms_output.put_line('amd_spare_parts_pkg: $Revision:   1.113  $') ;
    end version ;
