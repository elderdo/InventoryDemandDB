...
/*  need to clean up and streamline logic on this package */
    ...
    RESOURCE_BUSY constant number := -54 ;
    RETRY_LIMIT constant number := 4 ;
    debug boolean := false ;
    ...
    BEGIN
         writeMsg(pTableName => 'amd_partprime_pkg', 
                 pError_location => 390, pKey1 => 'amd_partprime_pkg', pKey2 => '$Revision:   1.27  $') ;
    END version ;
...
begin
	amd_owner.a2a_pkg.setDebug('Y') ;
	amd_owner.a2a_consumables_pkg.setDebug('Y') ;
	begin
		amd_owner.mta_truncate_table('amd_load_status','reuse storage') ;
		amd_owner.mta_truncate_table('amd_load_details','reuse storage') ;
	exception when others then
		null ; -- ignore any truncate error because these 
		       -- tables being empty are just for debug convenience.
	end ;
	--amd_owner.mta_truncate_table('tmp_a2a_part_info','reuse storage') ;
	setDebug('Y') ;
		-- this loop is only necessary to 
	-- accomodate a deadlock or resourse busy
	-- situation.
	while sleep_cnt < RETRY_LIMIT loop	
		begin
			dbms_output.put_line('diffPartToPrime:  started') ;
			diffPartToPrime ;
			sleep_cnt := RETRY_LIMIT ;
		exception when others then
			if sqlcode = DEADLOCK or sqlcode = RESOURCE_BUSY then
				sleep_cnt := sleep_cnt + 1 ;
				if sleep_cnt < RETRY_LIMIT then
					sleep(20) ;
				else				
					raise ;
				end if ;
...
prompt diffPartToPrime completed
