...
CREATE OR REPLACE PACKAGE AMD_OWNER.A2a_Pkg AS
 ...
 /*
      $Author:   zf297a  $
    $Revision:   1.6  $
     $Date:   16 Oct 2007 09:44:52  $
    $Workfile:   a2a_pkg.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\a2a_pkg.sql.-arc  $
/*   
/*      Rev 1.6   16 Oct 2007 09:44:52   zf297a
/*   Updated package with initA2APartAlt
/*   
...
/*   Initial revision.
 ...
 -- same name
     ...
     BEGIN
          writeMsg(pTableName => 'tmp_a2a_order_info_line', pError_location => 520,
           ...
           LOOP
              ...
              insertTmpA2AOrderInfoLine(rec.gold_order_number,rec.loc_sid,rec.order_date,
                rec.part_no,rec.order_qty, 
                rec.sched_receipt_date, rec.action_code,
                rec.line) ;
                cnt := cnt + 1 ;
          END LOOP ;
    ...
    exception when standard.no_data_found then
     ...
     begin
       IF amd_utils.isSpoPrimePart(part_no)
          and (
                  ( 
                    action_code = amd_defaults.DELETE_ACTION
                    and ( 
     ...
     PROCEDURE initA2ABomDetail(from_dt IN DATE := START_DT, to_dt IN DATE := SYSDATE ) IS
            ...
            FOR step IN startStep..endStep LOOP
                ...
                ELSIF step = 11 THEN
                     IF NOT Amd_Batch_Pkg.isStepComplete(batch_job_number => theJob, system_id => loadAll.system_id,
                        ...
                        amd_lp_override_consumabl_pkg.loadAllA2A ;
                    END IF ;
                ELSIF step = 12 THEN
                     IF NOT Amd_Batch_Pkg.isStepComplete(batch_job_number => theJob, system_id => loadAll.system_id,
        ...
        BEGIN
             writeMsg(pTableName => 'a2a_pkg', 
                     pError_location => 1640, pKey1 => 'a2a_pkg', pKey2 => '$Revision:   1.6  $') ;
              dbms_output.put_line('a2a_pkg: $Revision:   1.6  $') ;
        END version ;
