...
/*
      $Author:   zf297a  $
    $Revision:   1.2  $
        $Date:   03 Oct 2007 13:45:02  $
    $Workfile:   amd_demand.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\amd_demand.sql.-arc  $
/*   
/*      Rev 1.2   03 Oct 2007 13:45:02   zf297a
/*   Updated the script with the new package
...
--  Then read them with the new_part_no being passed
  ...
  END;
	...
	FUNCTION doDmndFrcstConsumablesDiff(
        ...
        procedure insertA2A is
            part_no amd_spare_parts.part_no%type := amd_utils.getPrimePartNo(nsn) ; 
            --cur_period date := amd_part_loc_forecasts_pkg.getCurrentPeriod ; -- comes from amd_param_changes
            calendar_date date := getCalendarDate(period) ;
            cur_period  number := getFiscalPeriod(sysdate) ;            
        begin
            if calendar_date is not null and duplicate is not null and period >= cur_period then
                if part_no is not null 
                and amd_utils.isPartActive(part_no) 
                and a2a_consumables_pkg.isPartValid(part_no) 
                and a2a_pkg.wasPartSent(part_no) then
                
                    insert into tmp_a2a_ext_forecast
                    (part_no, location, demand_forecast_type, period, quantity, action_code, last_update_dt, duplicate)
                    values (part_no, sran,EXTERNAL, calendar_date,
                    demand_forecast, action_code, sysdate, duplicate) ;
        ...
        exception 
            when standard.DUP_VAL_ON_INDEX then
                return;
            when others then 
            errorMsg(sqlFunction => 'insert', tableName => 'tmp_a2a_ext_forecast', 
                pError_location => 100,
                        key1 => nsn,
                        key2 => sran, 
                        key3 => period) ;
    ...
    begin
        for rec in demandsNotSame loop
            if nsn is null then
                nsn := rec.nsn ;
                sran := rec.sran ;
    ...
    procedure loadAllA2A is
        cursor dmndFrcstConsumables is 
        select * from amd_dmnd_frcst_consumables ;
        cnt number ;
        
        procedure insertA2A(rec in amd_dmnd_frcst_consumables%rowtype) is
            part_no amd_spare_parts.part_no%type := amd_utils.getPrimePartNo(rec.nsn) ; 
            --cur_period date := amd_part_loc_forecasts_pkg.getCurrentPeriod ; -- comes from amd_param_changes
            calendar_date date := getCalendarDate(rec.period) ;
            cur_period number := getFiscalPeriod(sysdate) ;
        begin
            if rec.period >= cur_period and calendar_date is not null and rec.duplicate is not null then
                if part_no is not null 
                and amd_utils.isPartActive(part_no) 
                and a2a_consumables_pkg.isPartValid(part_no) 
                and a2a_pkg.wasPartSent(part_no) then
                    ...
                    insert into tmp_a2a_ext_forecast
                    (part_no, location, demand_forecast_type, period, quantity, action_code, last_update_dt, duplicate)
                    values (part_no, rec.sran,EXTERNAL, calendar_date,
                    rec.demand_forecast, rec.action_code, sysdate, rec.duplicate) ;
        ...
        exception 
            when standard.DUP_VAL_ON_INDEX then
                ...
                set demand_forecast_type = EXTERNAL,
                quantity = rec.demand_forecast,
                action_code = rec.action_code,
                duplicate = rec.duplicate,
                last_update_dt = sysdate    
                where  part_no = insertA2A.part_no
                and location = rec.sran
                and period = calendar_date ;
    ...
    begin
        writeMsg(pTableName => 'tmp_a2a_ext_forecast', pError_location => 999,
        pKey1 => 'loadAllA2A',
        pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;

        for rec in dmndFrcstConsumables loop
            insertA2A(rec) ;
            cnt := cnt + 1 ;
        end loop ;
	...
	begin
		 writeMsg(pTableName => 'amd_demand', 
		 		pError_location => 10, pKey1 => 'amd_demand', pKey2 => '$Revision:   1.2  $') ;
	end version ;
