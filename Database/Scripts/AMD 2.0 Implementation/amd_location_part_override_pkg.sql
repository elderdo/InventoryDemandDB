...
CREATE OR REPLACE PACKAGE AMD_OWNER.Amd_Location_Part_Override_Pkg AS
 /*
      $Author:   zf297a  $
	$Revision:   1.5  $
        $Date:   16 Oct 2007 09:30:54  $
    $Workfile:   amd_location_part_override_pkg.sql  $
         $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\amd_location_part_override_pkg.sql.-arc  $
/*   
/*      Rev 1.5   16 Oct 2007 09:30:54   zf297a
/*   Updated checkForDeletedSpoPrimeParts
/*   
...
/*   Initial revision.
	...
	FUNCTION insertedTmpA2ALPO (
			...
			begin
                    ...
                    or pTslOverrideQty is null then
                        return ;
                    end if ;                        
			...
			for rec in cur_stock loop
				...
				exception when others then
					errorMsg(pSqlfunction => 'LoadUk',pTableName => 'tmp_amd_location_part_override',
					   pError_location => 240, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'qty: ' || rec.tsl_override_qty) ;
					raise ;
			   	end ;
				stock_cnt := stock_cnt + 1 ;
			end loop ;
            
			for rec in cur_cand loop
				tslOverrideQty := 0 ;
				begin
				...
				exception when others then
				  errorMsg(pSqlfunction	=> 'LoadUk',pTableName => 'tmp_amd_location_part_override',
					   pError_location => 250, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'locSid: ' || rec.loc_sid) ;
					   raise ;
			   	end ;
				cand_cnt := cand_cnt + 1 ;
			end loop ;
	...
	begin
		...
		begin
			for rec in cur_stock loop
				...
				exception when others then
					ErrorMsg(pSqlfunction => 'LoadAUS', pTableName => 'tmp_amd_location_part_override',
					   pError_location => 290, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'qty: ' || rec.tsl_override_qty) ;
					raise ;
			   	end ;
				stock_cnt := stock_cnt + 1 ;
			end loop ;
			...
			for rec in cur_cand loop
				...
				exception when others then
				  errorMsg(pSqlfunction => 'LoadAUS', pTableName => 'tmp_amd_location_part_override',
					   pError_location => 300, pKey1 => 'partNo: '|| rec.part_no, pKey2 => 'locSid: '|| rec.loc_sid) ;
					   raise ;
			   	end ;
				cand_cnt := cand_cnt + 1 ;
			end loop ;
	...
	begin
		...
		begin
			for rec in cur_stock loop
				...
				exception when others then
					ErrorMsg(pSqlfunction => 'LoadCAN', pTableName => 'tmp_amd_location_part_override',
					   pError_location => 340, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'qty: ' || rec.tsl_override_qty) ;
					raise ;
			   	end ;
				stock_cnt := stock_cnt + 1 ;
			end loop ;
			...
			for rec in cur_cand loop
				...
				exception when others then
				  errorMsg(pSqlfunction => 'LoadCAN', pTableName => 'tmp_amd_location_part_override',
					   pError_location => 350, pKey1 => 'partNo: '|| rec.part_no, pKey2 => 'locSid: '|| rec.loc_sid) ;
					   raise ;
			   	end ;
				cand_cnt := cand_cnt + 1 ;
			end loop ;
	...
	begin
		...
		begin
			for rec in cur_stock loop
				...
				exception when others then
					errorMsg(pSqlfunction => 'LoadBasc', pTableName => 'tmp_amd_location_part_override',
					   pError_location => 390, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'qty: '|| rec.tsl_override_qty) ;
					   raise ;
			   	end ;
				stock_cnt := stock_cnt + 1 ;
			end loop ;
			...
			for rec in cur_cand loop
				...
				EXCEPTION WHEN OTHERS THEN
					ErrorMsg(pSqlfunction => 'LoadBasc',pTableName => 'tmp_amd_location_part_override',
					   pError_location => 400, pKey1 => 'partNo: ' || rec.part_no, pKey2 => 'locSid: ' || rec.loc_sid) ;
					raise ;
			   	end ;
				cand_cnt := cand_cnt + 1 ;
			end loop ;
	...
	BEGIN
		...
		for rec in cur_req loop
			...
			exception when others then
                errorMsg(pSqlfunction => 'LoadFslMob', pTableName => 'tmp_amd_location_part_override',
				   pError_location => 450, pKey1 => rec.part_no, pKey2 => rec.loc_sid) ;
				   raise ;
	  		end ;
			req_cnt := req_cnt + 1 ;
		end loop ;
	...
	procedure loadWhse(startStep in number := 1, endStep in number := 5) is
        ...
        curStep number := 0 ;
            ...
            begin
    			...
    			for rec in cursor_peacetimeBasesSum LOOP
            ...
            begin
        		writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 510,
       			...
       			for rec in cursor_wartimeRspSum loop
    				begin
            ...
            begin
        		...
        		for rec in cursor_peacetimeBO_Spo_Sum loop
    				begin
            ...
            begin
    			...
    			for rec in cursor_peacetimeSpoInv loop
                    part_no := rec.part_no ;
                    if amd_utils.ISNUMBER(rec.qty) then
        				lineNo := 1; partNoSpoInv_sum(rec.part_no) := rec.qty ;
        				lineNo := 2 ;spoInv_cnt := spoInv_cnt + 1 ;
                    else
                		lineNo := 3;
                        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 580,
                				pKey1 => 'loadPeaceTimeSpoInv',
                				pKey2 => 'rec.part_no=' || rec.part_no,
    	...
    	--		wareHouseLocSid := amd_utils.GetLocSid(amd_defaults.AMD_WAREHOUSE_LOCID) ;
    			...
    			for rec in cursor_warehouse_parts loop
    				begin
    					begin
    				...
    				exception when others then
    				   ...
    				   pError_location => 620, pKey1 => 'partNo: ' || rec.part_no) ;
    				   raise ;
    			    end ;
    				cur_cnt := cur_cnt + 1 ;
    			end loop ;
		...
		writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 700,
		 ...
		 for rec in cur loop
			...
			exception when others then
				   ...
				   pError_location => 710,pKey1	=> 'nsiSid: ' || rec.nsi_sid,pKey2 => 'partNo: ' || rec.part_no) ;
				   raise ;
			end ;
			cur_cnt := cur_cnt + 1 ;
		 end loop ;
		 ...
		 loop
			...
			exception when others then
				errorMsg(pSqlfunction => 'processTsl',pTableName => 'tmp_amd_location_part_override',
				   pError_location => 750,pKey1 => 'spo_prime_part: ' || rec.spo_prime_part_no,
                    pKey2 => 'action_code: ' || rec.action_code, pKey3 => 'spo_location: ' || rec.spo_location,
    ...
    begin
         writeMsg(pTableName => 'amd_location_part_override_pkg', 
                 pError_location => 1360, pKey1 => 'amd_location_part_override_pkg', pKey2 => '$Revision:   1.5  $') ;
         dbms_output.put_line('amd_location_part_override_pkg: $Revision:   1.5  $') ;
    end version ;
