...
CREATE OR REPLACE PACKAGE AMD_OWNER.amd_defaults as
    /*
     ...
     $Author:   zf297a  $
   $Revision:   1.3  $
       $Date:   12 Oct 2007 17:36:26  $
   $Workfile:   amd_defaults.sql  $
        $Log:   I:\Program Files\Merant\vm\win32\bin\pds\archives\SDS-AMD\Database\Scripts\AMD 2.0 Implementation\amd_defaults.sql.-arc  $
/*   
/*      Rev 1.3   12 Oct 2007 17:36:26   zf297a
/*   Added new constants for various loc_id's and sc's
   ...
   Added PVCS keyword $Log$ and copied the PVCS history log into the header comments. 
	...
	begin
		 writeMsg(pTableName => 'amd_defaults', 
		 		pError_location => 10, pKey1 => 'amd_defaults', pKey2 => '$Revision:   1.3  $') ;
		 dbms_output.put_line('amd_defaults: $Revision:   1.3  $') ;
	end version ;
