...
CREATE OR REPLACE PACKAGE AMD_OWNER.Amd_Inventory AS
   ...
   added pvcs comments with keywords
	...
	function getCurrentLine(gold_order_number in amd_on_order.gold_order_number%type, order_date in amd_on_order.order_date%type) return number ; 
...
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.Amd_Inventory AS
   ...
   added comments to document changes made 
	...
	BEGIN
        ...
        writeMsg(pTableName => 'tmp_amd_...', pError_location => 20,
			...
			pkey3 => 'cntOnHandInvs=' || TO_CHAR(cntOnHandInvs),
			pkey4 => 'cntInRepair=' ||TO_CHAR(cntInRepair),
			pData => 'cntOnOrder=' || TO_CHAR(cntOnOrder),
			pComments => 'cntInTransits=' || TO_CHAR(cntInTransits) || ' cntInRsp=' || TO_CHAR(cntlnRsp)) ;
	...
	PROCEDURE loadOnOrder IS
		...
		FROM (				
			...
			WHERE
				   o.status = 'O'
				   AND o.order_type = 'C'
				   AND SUBSTR(o.order_no,1,2) IN ('FC','BA','AM','RS','SE','BR','BN','LB','DU','CF','NF')
			UNION ALL
			SELECT 	
					part part_no,
	  				SUBSTR(sc,8,6) loc_id,
					...
					WHERE
						 ...
						 and substr(asp.loc_id,1,2) in ('FB','EY')) invQ,
			    AMD_SPARE_NETWORKS  asn,
				AMD_SPARE_PARTS  asp,
				AMD_SPARE_NETWORKS  asnLink
		WHERE
	...
	BEGIN
        ...
        writeMsg(pTableName => 'tmp_amd_in_transits', pError_location => 50,
        ...
        writeMsg(pTableName => 'tmp_amd_in_transits', pError_location => 60,
            pKey1 => 'loadInTransits',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
	END loadInTransits ;
	...
	BEGIN
        ...
        writeMsg(pTableName => 'tmp_amd_rsp', pError_location => 70,
            pKey1 => 'loadRsp',
            pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
	...
	FOR rec IN partCur LOOP
        ...
        writeMsg(pTableName => 'tmp_amd_rsp', pError_location => 80,
            pKey1 => 'loadRsp',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
	END loadRsp;
	...
	BEGIN
        ...
        A2a_Pkg.insertTmpA2AOrderInfoLine(gold_order_number,
              ...
              getCurrentLine(gold_order_number, order_date)) ;
	...
	BEGIN
		...
		begin
			 ...
			 IF Amd_Location_Part_Override_Pkg.insertedTmpA2ALPO(pPartNo => part_no, 
                    pBaseName => rsp_location,
                    pOverrideType => override_type, pTslOverrideQty => qty, 
                    pOverrideReason => amd_location_part_override_pkg.OVERRIDE_REASON,
                    pTslOverrideUser => Amd_Location_Part_Override_Pkg.GetFirstLogonIdForPart(Amd_Utils.GetNsiSidFromPartNo(part_no)),
                    pBeginDate => SYSDATE, pActionCode => a2a_action_code, pLastUpdateDt => SYSDATE)  THEN
                    							
                NULL ; -- do nothing
                
			 END IF ;
             if override_type = amd_lp_override_consumabl_pkg.ROP_TYPE
             and amd_utils.isPartConsumable(part_no) then
                -- amd_rsp_sum does not store the ROQ so send the default value for it
                 IF Amd_Location_Part_Override_Pkg.insertedTmpA2ALPO(pPartNo => part_no, 
                        pBaseName => rsp_location,
                        pOverrideType => amd_lp_override_consumabl_pkg.ROQ_TYPE, 
                        pTslOverrideQty => amd_defaults.ROQ , 
                        pOverrideReason => amd_location_part_override_pkg.OVERRIDE_REASON,
                        pTslOverrideUser => Amd_Location_Part_Override_Pkg.GetFirstLogonIdForPart(Amd_Utils.GetNsiSidFromPartNo(part_no)),
                        pBeginDate => SYSDATE, pActionCode => a2a_action_code, pLastUpdateDt => SYSDATE)  THEN
   ...
   FUNCTION InsertRow(
	...
	BEGIN
        writeMsg(pTableName => 'tmp_amd_on_hand_invs', pError_location => 90,
	   ...
	   dbms_output.put_line('loadOnHandInvs started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MM:SS')) ;
        ...
        writeMsg(pTableName => 'tmp_amd_on_hand_invs', pError_location => 100,
            pKey1 => 'loadOnHandInvs',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
	...
	BEGIN
        writeMsg(pTableName => 'tmp_amd_in_repair', pError_location => 110,
            pKey1 => 'loadInRepair',
            pKey2 => 'started at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM') ) ;
        ...
        writeMsg(pTableName => 'tmp_amd_in_repair', pError_location => 120,
            pKey1 => 'loadInRepair',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
			pkey3 => 'cntInRepair=' || TO_CHAR(cntInRepair),
			pkey4 => 'cntType=' || TO_CHAR(cntType2),
			pData => 'cntType4WholeSale=' || TO_CHAR(cntType4WholeSale),
			pComments => 'cntTypeBascWholeSale=' || TO_CHAR(cntTypeBASCWholeSale) 
                || ' cntType5WholeSale=' || TO_CHAR(cntType5WholeSale) ) ;
	...
	BEGIN
		 writeMsg(pTableName => 'amd_inventory', 
		 		pError_location => 130, pKey1 => 'amd_inventory', pKey2 => '$Revision:   1.90  $') ;
	END version ;
