...
*/
    ...
    begin
        return part_no in ('17P2A5974-519', '17P1B2630-1','M27500-20TN2G06','2609997',
    ...
    begin
        if tsl_override_qty is not null then
            if isTestPart(part_no) then
                dbms_output.put_line('inserting ' || part_no || ' for location ' || spo_location) ;
            end if ;                
            ...
            if isTestPart(part_no) then
                dbms_output.put_line('tsl_override_qty was null for ' || part_no || ' for location ' || spo_location) ;
            end if ;                
    ...
    procedure loadWhse  is
        ...
        cursor lvls_by_base is
            ...
            case
                when nvl(a.approved_lvl_qty,0) > 0 
                    then a.approved_lvl_qty - nvl(a.reorder_point,0)
        ...
        begin
            ...
            begin
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    dbms_output.put_line(rec.spo_prime_part_no ||': whse_rop=' || whse_rop || ' whse_roq=' || whse_roq || ' rec.rop= ' || rec.rop || ' rec.roq=' || rec.roq) ;
                end if ;
                if recFound then
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    dbms_output.put_line(rec.spo_prime_part_no  ||': rop= ' || rop || ' *** roq=' || roq || ' ***') ;
                end if ;
            ...
            */ 
            ...
            if rop = amd_defaults.ROP and roq = amd_defaults.ROQ then
                if isTestPart(rec.spo_prime_part_no) then
                    dbms_output.put_line(rec.spo_prime_part_no || ' rejected') ;
                end if ;
            else
                ...
                if isTestPart(rec.spo_prime_part_no) then
                    dbms_output.put_line(rec.spo_prime_part_no ||  ' accepted') ;
                end if ;
            end if ;      
        exception when others then
        ...
        begin        
            ...
            if whsePartRecs.first is not null then            
                for indx in whsePartRecs.first .. whsePartRecs.last    
                loop
                    if isTestPart(whsePartRecs(indx).spo_prime_part_no) then
                        dbms_output.put_line(whsePartRecs(indx).spo_prime_part_no || ': rop= ' || whsePartRecs(indx).rop || ' roq=' || whsePartRecs(indx).roq) ;
                    end if ;
                    sumRecs(whsePartRecs(indx).spo_prime_part_no).spo_prime_part_no := whsePartRecs(indx).spo_prime_part_no ;
                    if whsePartRecs(indx).rop <> amd_defaults.ROP then
                        sumRecs(whsePartRecs(indx).spo_prime_part_no).rop :=  
                            sumRecs(whsePartRecs(indx).spo_prime_part_no).rop + whsePartRecs(indx).rop ;
                    end if ;
                    
                    sumRecs(whsePartRecs(indx).spo_prime_part_no).roq :=  
                        sumRecs(whsePartRecs(indx).spo_prime_part_no).roq + whsePartRecs(indx).roq ;
                    if isTestPart(whsePartRecs(indx).spo_prime_part_no) then
                        dbms_output.put_line(whsePartRecs(indx).spo_prime_part_no || ': sum rop= ' || sumRecs(whsePartRecs(indx).spo_prime_part_no).rop || ' sum roq=' || sumRecs(whsePartRecs(indx).spo_prime_part_no).roq) ;
                    end if ;
                end loop ;
                commit ;
            end if ;
   ...
   begin
        ...
        dbms_output.put_line('process wesmParts') ;                
        open wesmParts ;
        ...
        if wesmRecs.first is not null then        
            for indx in wesmRecs.first .. wesmRecs.last loop
                ...
                if isTestPart(wesmRecs(indx).spo_prime_part_no) then
                    dbms_output.put_line(wesmRecs(indx).spo_prime_part_no || ': rop= ' || wesmRecs(indx).rop || ' roq=' || wesmRecs(indx).roq) ;
                end if ;
                wesmSumRecs(wesmRecs(indx).spo_prime_part_no).spo_prime_part_no := wesmRecs(indx).spo_prime_part_no ;
                wesmSumRecs(wesmRecs(indx).spo_prime_part_no).rop := wesmRecs(indx).rop ;
                wesmSumRecs(wesmRecs(indx).spo_prime_part_no).roq := wesmRecs(indx).roq ;
            end loop ;
        ...
        dbms_output.put_line('process wesm parts whsePart for UK') ;                
        ...
        dbms_output.put_line('process wesm parts whsePart for BASC') ;                
        ...
        dbms_output.put_line('processing wesmSumRecs') ;
        partIndx := wesmSumRecs.first ;
        while partIndx is not null loop
            if isTestPart(wesmSumRecs(partIndx).spo_prime_part_no) then
                dbms_output.put_line(wesmSumRecs(partIndx).spo_prime_part_no || ': rop= ' || wesmSumRecs(partIndx).rop || ' roq=' || wesmSumRecs(partIndx).roq) ;
            end if ;
        ...
        dbms_output.put_line('wesmSumRecs.count=' || wesmSumRecs.count) ;
        
        dbms_output.put_line('process lvls_by_base') ;                
        open lvls_by_base ;
        fetch lvls_by_base bulk collect into lvlsByBaseRecs ;
        close lvls_by_base ;
        
        if lvlsByBaseRecs.first is not null then
            for indx in lvlsByBaseRecs.first .. lvlsByBaseRecs.last loop
                in_cnt := in_cnt + 1 ;
                nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).spo_prime_part_no := lvlsByBaseRecs(indx).spo_prime_part_no ;
                nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).rop := lvlsByBaseRecs(indx).rop ;
                nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).roq := lvlsByBaseRecs(indx).roq ;
                if isTestPart(lvlsByBaseRecs(indx).spo_prime_part_no) then
                    dbms_output.put_line('indx part: ' || lvlsByBaseRecs(indx).spo_prime_part_no
                    || ' non-wesm part: ' || nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).spo_prime_part_no 
                    || ' sum rop= ' || nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).rop 
                    || ' sum roq=' || nonwesmSumRecs(lvlsByBaseRecs(indx).spo_prime_part_no).roq
                    || ' rop= ' || lvlsByBaseRecs(indx).rop
                    || ' roq=' || lvlsByBaseRecs(indx).roq) ;
                end if ;
            end loop ;
        end if ;            
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 140,
                pKey1 => 'loadWhse',
                pKey2 => 'at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
                pKey3 => 'lvlsByBaseRecs.count=' || to_char(lvlsByBaseRecs.count)) ;

        dbms_output.put_line('process non-wesm parts whsePart for UK') ;                
        processWhseParts(spo_location => amd_defaults.AMD_UK_LOC_ID, 
                            sc => amd_defaults.AMD_UK_SC, 
                            operator => 'like', wesm_part =>'N',
                            sumRecs => nonwesmSumRecs ) ;
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 150,
                pKey1 => 'loadWhse',
                pKey2 => 'at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
                pKey3 => 'processed nonwesm UK') ;
        
        dbms_output.put_line('process non-wesm parts whsePart for BASC') ;                
        processWhseParts(spo_location => amd_defaults.AMD_BASC_LOC_ID, 
                            sc => amd_defaults.AMD_BASC_SC, 
    ...
    begin
        ...
        writeMsg(pTableName => 'doLPOverrideConsumablesDiff', pError_location => 240,
           pKey1 => 'doUpdate.part_no=' || part_no,
           pKey2 => 'spo_location=' || spo_location,
           pKey3 => 'type=' || tsl_override_type,
    ...
    begin
     ...
     begin
        ...
        loadDefaultTsls ;
        writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 270,
            pKey1 => 'loadLocPartOverrides',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM')) ;
        commit ;
    ...
    begin
       ...
       writeMsg(pTableName => 'tmp_amd_location_part_override', pError_location => 290,
            pKey1 => 'loadDefaultTsls',
            pKey2 => 'ended at ' || TO_CHAR(SYSDATE,'MM/DD/YYYY HH:MI:SS AM'),
            pKey3 => 'in_cnt=' || to_char(in_cnt),
            pKey4 => 'insert_cnt=' || to_char(insert_cnt),
    ...
    begin
        writeMsg(pTableName => 'amd_lp_override_consumabl_pkg', 
             pError_location => 300, pKey1 => 'amd_lp_override_consumabl_pkg', pKey2 => '$Revision:   1.41  $') ;
        dbms_output.put_line('amd_lp_override_consumabl_pkg: $Revision:   1.41  $') ;
    end version ;
