...
END;
	...
	PROCEDURE doInsertTmpA2ABackOrderInfo (
			  part_no IN TMP_A2A_BACKORDER_INFO.part_no%TYPE,
			   loc_sid IN NUMBER, 
	...
	EXCEPTION
	    ...
	    WHEN OTHERS THEN 
	        ErrorMsg(pSqlfunction => 'insert',
			     pTableName => 'tmp_a2a_backorder_info', pError_location => 70,
	...
	END doInsertTmpA2ABackOrderInfo ;
	  ...
	  FUNCTION InsertRowBackorder(
		...
		BEGIN
			...
			action_code = Amd_Defaults.INSERT_ACTION,
			last_update_dt = SYSDATE
			WHERE spo_prime_part_no = InsertRowBackorder.spo_prime_part_no AND loc_sid = InsertRowBackorder.loc_sid;
	  ...
	  BEGIN
		...
		BEGIN
			...
			line_no := line_no + 1; rec.loc_sid := insertRowBackorder.loc_sid ;
		...
		IF (qty > 0) and spo_prime_part_no is not null THEN  
			...
			<<validateTmpA2ABackorderInfo>>
			DECLARE
				site_location TMP_A2A_BACKORDER_INFO.site_location%TYPE := Amd_Utils.GetSpoLocation(loc_sid);
			BEGIN
				 IF A2a_Pkg.isPartValid(spo_prime_part_no) 
				 AND A2a_Pkg.wasPartSent(partNo => spo_prime_part_no) 
				 AND site_location IS NOT NULL THEN
				 	 <<insertTmpA2ABackorderInfo>>
				 	-- declare
					 	-- spo_prime_part_no TMP_A2A_BACKORDER_INFO.part_no%TYPE  := Amd_Utils.getSpoPrimePartNo(spo_prime_part_no) ;
					 begin
						 if spo_prime_part_no is not null then
						 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => loc_sid, site_location => site_location,
							 							 qty => qty, 
														 action_code => Amd_Defaults.INSERT_ACTION) ;
						 end if ;
					 end insertTmpA2ABackorderInfo ;
				 END IF ;
			END validateTmpA2ABackorderInfo;
	...
	BEGIN
	  ...
	  BEGIN
		   ...
		   AND loc_sid = UpdateRowBackorder.loc_sid;
	
	  
		  IF (qty > 0)  THEN	  
			  <<validateTmpA2ABackoderInfo>>
			  DECLARE
			     site_location TMP_A2A_BACKORDER_INFO.site_location%TYPE := Amd_Utils.GetSpoLocation(loc_sid);	
			  BEGIN
			    IF site_location IS NOT NULL  
				AND A2a_Pkg.isPartValid(spo_prime_part_no)
				AND A2a_Pkg.wasPartSent(spo_prime_part_no) THEN
					<<insertTmpA2ABackorderInfo>>
					--declare
				 		   --spo_prime_part_no TMP_A2A_BACKORDER_INFO.part_no%TYPE := Amd_Utils.getSpoPrimePartNo(part_no);		 
					begin
					 if spo_prime_part_no  IS NOT NULL then
					 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => loc_sid, site_location => site_location,
						 							 qty => qty,
	...
	END UpdateRowBackorder;
	 ...
	 BEGIN
		...
		BEGIN
			  ...
			  AND loc_sid = DeleteRowBackorder.loc_sid ;
			...
			DECLARE
				   ...
				   --spo_prime_part_no TMP_A2A_BACKORDER_INFO.part_no%TYPE := Amd_Utils.getSpoPrimePartNo(part_no);
			BEGIN
			    IF site_location IS NOT NULL  AND  spo_prime_part_no IS NOT NULL THEN 
				 	 doInsertTmpA2ABackOrderInfo (part_no => spo_prime_part_no, loc_sid => loc_sid, site_location => site_location,
					 							 qty => 0,
												 action_code => Amd_Defaults.DELETE_ACTION) ;
				END IF;
		...
		exception when standard.no_data_found then
				 writeMsg(pTableName => 'amd_sent_to_a2a', 
				 		pError_location => 100, 
						pKey1 => 'spo_prime_part_no=' || spo_prime_part_no, 
						pKey2 => 'loc_sid=' || to_char(loc_sid),
						pKey3 => 'part no in amd_sent_to_a2a') ;
				return success ;
		END updateAmdBackorderSum;
	...
	BEGIN
		 writeMsg(pTableName => 'amd_reqs_pkg', 
		 		pError_location => 110, pKey1 => 'amd_reqs_pkg', pKey2 => '$Revision:   1.17  $') ;
	END version ;
