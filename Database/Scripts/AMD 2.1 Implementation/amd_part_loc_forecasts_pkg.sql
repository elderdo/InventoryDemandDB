...
-- will need to constantly diff after all to check for new and deleted parts --
	...
	*/  
	...
	PROCEDURE TmpA2A_EF_AddMonth(startDate DATE) IS
	     type extForecastRec is record (
            part_no amd_part_loc_forecasts.part_no%type,
            location amd_spare_networks.spo_location%type,
            period date,
            quantity amd_part_loc_forecasts.FORECAST_QTY%type,
            action_code amd_part_loc_forecasts.ACTION_CODE%type,
            last_update_dt amd_part_loc_forecasts.last_update_dt%type
         ) ;		 
         type extForecastTab is table of extForecastRec ;
         extForecastRecs extForecastTab ;
         
		 CURSOR extForecastCur IS
			 SELECT 
			 		part_no,
	...
	BEGIN		 
        open extForecastCur ;
        fetch extForecastCur bulk collect into extForecastRecs ;
        close extForecastCur ;
        if extForecastRecs.first is not null then
            FOR indx in extForecastRecs.first .. extForecastRecs.last 
            LOOP
                BEGIN
                  ...
                  -- which will not know till subsequent diff occurs.
                     IF (NOT amd_location_part_leadtime_pkg.IsPartDeleted(extForecastRecs(indx).part_no) ) THEN
    				 		 
                        InsertTmpA2A_EF
                         (	
                            extForecastRecs(indx).part_no,
                            extForecastRecs(indx).location,
                            DEMAND_FORECAST_TYPE,
                            extForecastRecs(indx).period,
                            extForecastRecs(indx).quantity,
                            extForecastRecs(indx).action_code,
                            extForecastRecs(indx).last_update_dt,
                ...
                EXCEPTION WHEN OTHERS THEN
                    returnCode := ErrorMsg(
                               ...
                               pError_location 	  => 30,
                               pReturn_code	  	  => 99,
                               pKey1			  => extForecastRecs(indx).part_no,
                               pKey2			  => extForecastRecs(indx).location,
                               pKey3			  => extForecastRecs(indx).period,		   
                               pData			  => '',
                               pComments		  => PKGNAME) ;		
                               RAISE ;
	...
	procedure processPartLocForecasts(partLocForecasts in partLocForecastsCur) is
        ...
        partLocForecastsRecs partLocForecastsTab ;
	...
	begin
         ...
         fetch partLocForecasts bulk collect into partLocForecastsRecs ;
         close partLocForecasts ;
         
         if partLocForecastsRecs.first is not null then
             for indx in partLocForecastsRecs.first .. partLocForecastsRecs.last                
             loop
                 InsertTmpA2A_EF_AllPeriods(
                                   partLocForecastsRecs(indx).part_no, 
                                   partLocForecastsRecs(indx).spo_location, 
                                   sdate, 
                                   partLocForecastsRecs(indx).forecast_qty, 
                                   partLocForecastsRecs(indx).action_code,
                                   sysdate ) ;
                 cnt := cnt + 1 ;
             end loop ;
         end if ;             
	...
	begin
		 writeMsg(pTableName => 'amd_part_loc_forecasts_pkg', 
		 		pError_location => 280, pKey1 => 'amd_part_loc_forecasts_pkg', pKey2 => '$Revision:   1.31  $') ;
		 dbms_output.put_line('amd_part_loc_forecasts_pkg: $Revision:   1.31  $') ;
	end version ;
