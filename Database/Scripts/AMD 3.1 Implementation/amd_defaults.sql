...
CREATE OR REPLACE PACKAGE AMD_OWNER.amd_defaults as
   ...
   added $Log$ PVCS keyword
    ...
    function getCONSUMABLE_PLANNER_CODE return amd_planners.PLANNER_CODE%type ;
	CONSUMABLE_LOGON_ID amd_planner_logons.LOGON_ID%type := null ;
    function getCONSUMABLE_LOGON_ID return amd_users.BEMS_ID%type ;
	REPAIRABLE_PLANNER_CODE amd_planners.PLANNER_CODE%type := null ;
    function getREPAIRABLE_PLANNER_CODE return amd_planners.PLANNER_CODE%type ;
	REPAIRABLE_LOGON_ID amd_planner_logons.LOGON_ID%type := null ;
    function getREPAIRABLE_LOGON_ID return amd_users.BEMS_ID%type ;
    ...
    function getVersion return varchar2 ; -- added 5/22/2008 by dse
...
CREATE OR REPLACE PACKAGE BODY AMD_OWNER.amd_defaults as
   ...
   Added PVCS keyword $Log$ and copied the PVCS history log into the header comments. 
	...
	begin
        if amd_utils.isPartConsumable(amd_utils.getPartNo(amd_utils.getNsiSid(pNsn => nsn))) then
		 	return CONSUMABLE_PLANNER_CODE ;
		elsif amd_utils.isPartRepairable(amd_utils.getPartNo(amd_utils.getNsiSid(pNsn => nsn))) then
	...
	begin
		 writeMsg(pTableName => 'amd_defaults', 
		 		pError_location => 10, pKey1 => 'amd_defaults', pKey2 => '$Revision:   1.41  $') ;
		 dbms_output.put_line('amd_defaults: $Revision:   1.41  $') ;
	end version ;
    ...
    function getCONSUMABLE_PLANNER_CODE return amd_planners.PLANNER_CODE%type is
    begin
        return CONSUMABLE_PLANNER_CODE ;
    end getCONSUMABLE_PLANNER_CODE ;
    
    function getCONSUMABLE_LOGON_ID return amd_users.BEMS_ID%type is
    begin
        return CONSUMABLE_LOGON_ID ;
    end getCONSUMABLE_LOGON_ID ;
    
    function getREPAIRABLE_PLANNER_CODE return amd_planners.PLANNER_CODE%type is
    begin
        return REPAIRABLE_PLANNER_CODE ;
    end getREPAIRABLE_PLANNER_CODE ;
    
    function getREPAIRABLE_LOGON_ID return amd_users.BEMS_ID%type is
    begin
        return REPAIRABLE_LOGON_ID ;
...
begin
	declare
        ...
        function getConsumablePlannerCode return amd_planner_logons.planner_code%type is
            planner_code amd_planner_logons.PLANNER_CODE%type ;
        begin
            select planner_code into getConsumablePlannerCode.PLANNER_CODE
        ...
        function getRepairablePlannerCode return amd_planner_logons.planner_code%type is
            planner_code amd_planner_logons.PLANNER_CODE%type ;
        begin
            select planner_code into getRepairablePlannerCode.PLANNER_CODE
            from amd_default_planners_v
        ...
        function getConsumableLogonId return amd_planner_logons.LOGON_ID%type is
            logon_id amd_planner_logons.LOGON_ID%type ;
        begin
            select bems_id into getConsumableLogonId.logon_id
        ...
        function getRepairableLogonId return amd_planner_logons.LOGON_ID%type is
            logon_id amd_planner_logons.LOGON_ID%type ;
        begin
            select bems_id into getRepairableLogonId.logon_id
            from amd_default_users_v
            where default_type = 'REPAIRABLE' ;
            return logon_id ;
        exception when standard.no_data_found then
       ...
       function getDateLvlLoadedModifier return varchar2 is
        ...
        function getFmsSegCodeForAUS return whse.sc%type is
            sc whse.sc%type ;
        begin
        ...
        function getFmsSegCodeForBASC return whse.sc%type is
            sc whse.sc%type ;
        begin
        ...
        function getFmsSegCodeForCAN return whse.sc%type is
            sc whse.sc%type ;
        begin
        ...
        function getFmsSegCodeForUK return whse.sc%type is
            sc whse.sc%type ;
        begin
	...
	begin
        ...
        fmsSegCodes := fmsSegCodeTab(AMD_AUS_SC,AMD_BASC_SC,AMD_CAN_SC,AMD_UK_SC) ;
