...
function checkLockFile {
  ...
  if [[ -f ${TRIM_TACTICS_LOCK_FILE} ]] ; then
    TRIMTACTICS_WARNING_MESSAGE="trimTactics.ksh is already running, because the lock file \
      ${TRIM_TACTICS_LOCK_FILE} $(ls -al ${TRIM_TACTICS_LOCK_FILE}) was found to exist.  \
      Delete it if there is no trimTactics.ksh script running.  The current process id is $$.  \
      Searching for other processes...: $(ps -elf | grep trimTactics.ksh | sed '/grep trimTactics/d')"
...
function analyzeTacticsLogFile {
...
function getTrimTacticsLogFile {
