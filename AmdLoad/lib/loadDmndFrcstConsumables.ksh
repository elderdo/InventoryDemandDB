...
#   Rev 1.12   14 Oct 2009
#   Rev 1.13   15 Feb 2018 removed obsolte back tic's and replaced them with $(...)
#
#
...
MesgFile=$LOG_HOME/loadDmndMsg.txt
...
function sendWarningMsg {
	...
	if (($#==0)) ; then
		print "loadDmndFrcstConsumables.ksh sendWarningMsg: missing required param"
		exit 4
	fi
...
function GenStats {
	...
	if [ -s $MesgFile ]
	then
	#	mailx -s "DmndFrcstConsumables Load Notification" -r loadDmndFrcstConsumables $MesgRecpt1 $MesgRecpt2 $MesgRecpt3 <$MesgFile
		cat $MesgFile
	fi
}
