CREATE OR REPLACE package amd_preferred_pkg
is
	/*
	...
	*/
	...
	function GetDlaWarehouseStock(pNsi_sid in amd_national_stock_items.nsi_sid%type) return amd_national_stock_items.dla_warehouse_stock%type;
	function GetDlaWarehouseStock(pNsn in amd_nsns.nsn%type) return amd_national_stock_items.dla_warehouse_stock%type;
...
/
CREATE OR REPLACE package body amd_preferred_pkg is
	...
	function GetDlaWarehouseStock(pNsi_sid in amd_national_stock_items.nsi_sid%type) return amd_national_stock_items.dla_warehouse_stock%type is
		dla_warehouse_stock amd_national_stock_items.dla_warehouse_stock%type ;
		dla_warehouse_stock_cleaned amd_national_stock_items.dla_warehouse_stock_cleaned%type ;
	begin
	...
	function GetDlaWarehouseStock(pNsn in amd_nsns.nsn%type) return amd_national_stock_items.dla_warehouse_stock%type is
	begin
	...
	begin
		select mtbdr, mtbdr_cleaned
		into mtbdr, mtbdr_cleaned
		from amd_national_stock_items
		where nsi_sid = pNsi_sid ;
		return GetPreferredValue(mtbdr_cleaned, mtbdr) ;
	end GetMtbdr ;
	...
	begin
		select planner_code, planner_code_cleaned
		into planner_code, planner_code_cleaned
		from amd_national_stock_items
		where nsi_sid = pNsi_sid ;
		return GetPreferredValue(planner_code_cleaned, planner_code) ;
	end GetPlannerCode ;
